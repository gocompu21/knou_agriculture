...
OCR_SYSTEM = (
    ...
    "수식·표는 모양을 살려 옮긴다 (판독 결과는 회원이 입력칸에서 그대로 보고 고친다):\n"
    "- 가로줄 위아래로 쓴 **분수**는 한 줄로 '분자 / 분모' 로 적는다. 분자·분모가\n"
    "  여러 항이면 괄호로 묶는다. 예: 1,000 × 1,000 / 40 = 25,000,\n"
    "  (3 + 2) / 2 = 2.5. [eq] 같은 표식이나 LaTeX 는 쓰지 않는다.\n"
    ...
    for up in uploads:
        ...
        for item in parsed.answers:
            ...
            if r is not None and r < 0.5:
                ...
                continue
            # 판독 결과는 입력칸에 그대로 들어가 회원이 읽고 고친다. 예전 프롬프트가
