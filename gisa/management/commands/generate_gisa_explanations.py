...
def get_image_parts(question):
    """문제/보기에 이미지가 있으면 (label, Part) 리스트를 반환한다."""
    image_fields = [
        ('text_image', '문제 이미지'),
    ...
    for field_name, label in image_fields:
        ...
        parts.append((label, types.Part.from_bytes(data=data, mime_type=mime)))
    return parts


def generate_explanation(client, question, model_name):
    prompt = build_prompt(question)
    image_parts = get_image_parts(question)

    if image_parts:
        # 멀티모달: 텍스트 + 이미지
        contents = [prompt]
        for label, part in image_parts:
            contents.append(f"\n[{label}]:")
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for i, question in enumerate(qs, 1):
            ...
            img_count = sum(1 for f in ['text_image', 'choice_1_image', 'choice_2_image', 'choice_3_image', 'choice_4_image'] if getattr(question, f))
            img_tag = f' [img:{img_count}]' if img_count else ''
            self.stdout.write(f'[{i}/{total}] {label}{img_tag} ... ', ending='')
