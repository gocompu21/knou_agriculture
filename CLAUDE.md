...
- **스케일자**(도구 줄 '스케일자', `#cadSr`, localStorage `cadSr` = {x, y, rot, roll}): **사용자 실물(사진·단면·실측)을 그대로 옮긴 삼각 스케일**(2026-09).
  단면은 한 변 25mm 정삼각형, 세 면 가운데에 둥근 홈(입구 약 4mm)을 파 면마다 색칠 — 면 하나에 축척 둘: **빨강 1/100·1/200 · 청록 1/300·1/400 ·
  ...
  **제도판에서는 한 면을 바닥에 눕히므로 위에서 보이는 것은 두 빗면과 가운데 능선이다**(사용자 지적 — 처음에 한 면을 평평하게 그렸다가 고쳤다).
  빗면은 60° 기울어 위에서 폭이 절반(12.5mm)으로 보이고 글자도 세로로 눌린다(`scale(1 .5)`). **종이에 닿는 두 모서리만** 치수를 재고 긋는
