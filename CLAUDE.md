...
- **자가 덮은 곳은 지워지지 않는다**(`covered` = I자 몸통·두 날 + 삼각자 전체 — **가운데 구멍은 막았다**(사용자 요청)).
...
- **삼각자 가운데 투명 회전 아이콘**(`.tri-rot`): 누르면 시계방향 90°(`rotateTri`). 실물처럼 다시 I자에 얹힌다 —
  90° 돌리면 짧은 변↔긴 변이 바뀌고 좌우가 뒤집히며, 그다음 방향은 삼각자를 뒤집어 놓아야 얹히므로 60° 자는
  (alt,flip) = (F,F)→(T,T)→(T,F)→(F,T) 네 방향을, 45° 자는 좌우 둘을 돈다. 무게중심 x 를 지킨다
...
- **삼각자 45° · 60°**(도구 줄 단추, 켜고 끄는 것 — 도구가 아니다): I자 윗날에 얹혀 좌우로만 민다(I자를
  ...
  아크릴 통판(처음 주황이었다가 '투명으로', 가운데 구멍도 '막을 것' 요청). **삼각자 위에서는 점·선·원 등 어느 도구도
  작용하지 않고 옮기기만 된다**(누르기만 해도 통과하지 않는다, 도구 미리보기도 없다) — 사용자 요청
  자리는 localStorage(`cadTri`)
- **투명 삼각자가 '없는 것처럼' 보이지 않게 실물 아크릴의 단서 셋**을 둔다(사용자 요청): ① 판 **바깥에만** 지는 옅은 그림자
  (evenodd clipPath `cadTriOut` — 판 밑까지 깔면 판 전체가 잿빛이 되어 도면이 흐려졌다) ② 가장자리 안쪽의 푸른 띠(두께감,
  `cadTriClip` 로 안쪽만) ③ 두 직각변의 종이 위 1cm·5mm 눈금. 가운데는 맑게 둔다. 그림자는 `pointer-events:none` — 손 커서가 번지지 않게
...
- **선택 도구로 빈 곳을 끌면 화면 이동(pan)이다**(사용자 요청, 2026-09). **영역 선택은 Shift+끌기** — AutoCAD 처럼
  ...
  지울 때 점은 **남은 객체가 쓰지 않을 때만** 지운다. 점 위에서 끌면 점을 옮긴다
