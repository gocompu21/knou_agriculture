...
| 탭 | 내용 |
|----|------|
| `textbook` 쪽집게 노트 | 학습전략 + `GisaEssayNote` 정리 자료 카드 |
| `study` 기출학습 | 빈출(4·3·2회)·재출제 유력·계산 학습 + 회차별 학습(정답 보며) |
| `solve` 기출고사 | 회차별 풀기(90분)·시험지(인쇄→사진) |
| `mock` 모의고사 | `source='모의'` — 기출 전체에서 topic_key 가 겹치지 않게 15문항 무작위(`_pick_mock`, 최근 연도 가중). 순번으로 표시 |
| `wrong` 오답노트 | `_wrong_attempts()` — 문항마다 **가장 최근 응시**로 판정, 만점 미달만. 다시 풀어 만점이면 빠진다. "오답 재풀이"는 `source='오답'`(최대 20문항, 타이머 없음) |
