...
- **스케일자**(도구 줄 '스케일자', `#cadSr`, localStorage `cadSr` = {x, y, rot, roll}): **사용자 실물(사진·단면·실측)을 그대로 옮긴 삼각 스케일**(2026-09).
  ...
  **면의 밝기는 빛 방향(화면 왼쪽 위)으로 정한다**(`darkA`) — 면 A 의 바깥쪽을 화면 방향으로 재어 빛을 등지면 A 를 어둡게, B 를 밝게. 예전에는 돌린 각도와 상관없이 A 가 밝아 90° 돌리면 그림자가 반대로 보였다(사용자). 능선 너머 그늘도 어두운 면 쪽. 바닥 그림자는 전부터 늘 오른쪽 아래
