...
- **스케일자**(도구 줄 '스케일자', `#cadSr`, localStorage `cadSr` = {x, y, rot, roll}): **사용자 실물(사진·단면·실측)을 그대로 옮긴 삼각 스케일**(2026-09).
  ...
  **샤프로 눈금 모서리를 끌지 않고 누르면 눈금 표시**(`srMark`): 가장 가까운 작은 눈금 자리에서 눈금과 나란히 자 바깥(종이) 쪽으로 보조선(종이 위 `SR_MARK` 6mm)을 긋는다. 모서리마다 축척·0 자리·방향(`srS`·`srU0`·`srDu`·`srOut`)을 `srEdges` 가 싣는다
