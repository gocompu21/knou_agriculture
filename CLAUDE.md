...
- **스케일자**(도구 줄 '스케일자' + 축척 고르기 1:100~1:1000 · **사용자 지정**(1:N 을 물어 목록에 더한다, localStorage `cadSrCustom`), `#cadSr`, localStorage `cadSr`): 삼각 스케일의 한 면 — 길이 31cm·폭 24mm(종이 위),
  윗변에 고른 축척 눈금, 가운데 붉은 띠. **눈금 1m(실제) = 종이 1000/S mm = 도면 denom/S m**(1:430 종이에 1:500 자면 0.86m). 눈금은 종이 위
  1mm 안팎(작은)·8mm 안팎(숫자) 간격을 `niceStep` 으로 고른다. 템플릿처럼 `srMap`·`srLocal`(회전 포함), 몸통을 끌면 **0 눈금이 눈금에 붙고**,
