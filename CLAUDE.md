...
- **점 합치기**(`mergeCands`·`mergePts`·`ctxMenu`): 샤프로 점을 고른 뒤 **그 점에서 오른쪽 버튼** → 그 점에 이어진 선의 연장선(또는 선 위, 화면 6px 안)에 있는 점을 가까운 차례로 메뉴에 보이고, 고르면 고른 점을 그 점으로 합친다 — 그 점을 쓰던 것(`REF_KEYS`)이 모두 새 점을 가리키고 원래 점은 지운다. 길이 0 이 된 선·사각형·호는 버린다. **합친 뒤 번호를 다시 매긴다** — 차례는 그대로 두고 빈 번호만 메운다(P2 를 합치면 P3→P2 …, `remapPts`). 끝이 어긋나게 그린 선을 잇는 용도(사용자)
