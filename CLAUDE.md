...
- **스케일자**(도구 줄 '스케일자', `#cadSr`, localStorage `cadSr` = {x, y, rot, roll}): **사용자 실물(사진·단면·실측)을 그대로 옮긴 삼각 스케일**(2026-09).
  ...
  **종이에 닿는 두 모서리만** 치수를 잰다(`srEdges`; 능선은 떠 있어 못 쓴다). **스케일자로는 선을 긋지 않는다**(사용자) —
  ...
  숫자 위 **작은 숫자는 반대쪽에서 읽는 값**이다. 실물은 긴 눈금이 작은 숫자 한가운데를 지나지만 **숫자가 눈금에 닿지 않게**(사용자) 작은 숫자는 긴 눈금 **옆**에, 큰 숫자는 눈금 끝 **아래**에 띄운다(종이 쪽은 화면 mm 로 정해 면 좌표로 바꾼다). 눈금 표 `SRG` = [작은, 중간, 긴 중간, 숫자] —
