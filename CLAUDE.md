...
세션만). 곧게 편 사진 위에 SVG 로 줄 긋기·물결·밑줄·✓·번호와 첨삭 글·빠짐·점수를
겹친다. 좌표는 편 사진 픽셀(2100×2970)이라 `viewBox` 를 같게 두면 화면 폭을 따라간다.
...
- 첨삭 글은 박스의 **남은 빈 줄**에 적고, 모자라면 사진 아래 목록으로 넘긴다
- 점수는 박스 오른쪽 위 인쇄 배점 '(5.0점)' **왼쪽**에 적는다(겹쳤다)
- 휴대폰에서는 쪽이 300px 남짓이라 글씨가 안 읽힌다. 쪽을 누르면 900px 로 넓혀 옆으로 민다
