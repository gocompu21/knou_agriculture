...
- **영역 선택**(선택 도구에서 빈 곳 끌기): AutoCAD 처럼 끄는 방향으로 가른다 — → 는 상자 안에
  다 든 것만(파란 실선), ← 는 걸친 것까지(초록 점선). 상자 안의 점도 함께 고르고, 지울 때 점은
  **남은 객체가 쓰지 않을 때만** 지운다. 그래서 선택 도구의 화면 이동은 빈 곳 끌기가 아니라
  오른쪽·휠 버튼 끌기나 Space+끌기다
