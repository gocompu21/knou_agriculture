...
**부분 점수는 항목 안에서도 준다**(`credit`, 2026-09). 예전에는 "부분적으로만 맞으면
matched=false" 라 항목 단위로 전부 아니면 전무였고, 한 가지만 묻는 서술형처럼
**기준표가 한 항목뿐인 문항은 0점 아니면 만점**만 나왔다. 이제 모델이 항목마다
`matched` 와 함께 `credit`(0.0~1.0)을 매기고, 점수는 `항목 배점 × credit` 을 더해
파이썬이 만든다. 단답·빈칸은 1.0/0.0만 주도록 프롬프트에 못박았다 — 맞거나 틀리거나
둘뿐인 유형이다.

> **점수 산술은 모델에게 맡기지 않는다.** 모델은 판정(`matched`·`credit`)만 하고
> 곱셈과 합산은 파이썬이 한다. `credit` 이 범위를 벗어나면(3.0·-1.0) 잘라 쓰고,
> `matched=true` 인데 `credit` 을 빠뜨리면 1.0 으로 본다 — 실제로 기본값 0.0 이
> 그대로 오는 경우가 있다.
>
> 화면은 부분 인정을 **△ '부분 인정'** 으로 그린다(`.pt.half`·`.rs-pt.half`).
> ✕ 로 두면 점수가 붙은 까닭을 알 수 없다.
