...
- **제도판은 실물 A1 제도판 900×600mm 고정**(`BOARD_W`·`BOARD_H`, 사용자) — 종이를 무엇으로 고르든 판 크기는 같고 종이는 판 가운데에 실제 비율로 붙는다(종이 mm → 도면 m 는 축척). 레일·펜 받침 치수는 판 단위 `u`(6mm)로 센다. **I자는 판 위 틀 밑 ~ 펜 받침 위까지 오르내린다**(`setBarY`; 종이 범위로 막았더니 종이 아래로 못 내려갔다). '맞춤'은 판 전체가 아니라 **종이 + 여백**을 잡는다(판을 다 잡으면 종이가 작아진다). 예전에는 판이 종이 크기에 비례했다.
...
- **삼각자 크기는 실물 cm**(9·12·15·18·24·30, **기본 12cm**). **60° 자는 따로 — 직각을 낀 긴 변 45cm**(사용자 지정, 설정의 '60° 삼각자' 24·30·36·45, `SET.tri60Size`). 45° 자만 아래 크기 목록을 따른다. 시중 제도용 세트는 18~45cm 지만, 30cm 는 A3 모눈종이에서
  종이 밖으로 크게 나가 '너무 크다'는 지적을 받았다(처음의 5.6cm 는 '너무 작다'). 직각을 낀 긴 변의 길이이고,
  도면 축척(`sheet.scale` '1:430' → 1cm = 4.3m)으로 m 로 바꾼다(`scaleDenom`). **45° 자는 직각변을 굵은 눈금(5m) 배수로 맞춘다**
  (12cm → 51.6m → 50m) — 꼭짓점이 눈금 교차점에 오게. 처음에는 종이 폭 기준 임의 값
  (24U, 종이 위 약 5.6cm)이라 '너무 작다'는 지적을 받았다. 30cm 는 A3 모눈종이(401)에서 129m 라 종이 밖으로 나간다 — 실물 비율이다
...
- **스케일자**(도구 줄 '스케일자', `#cadSr`, localStorage `cadSr` = {x, y, rot, roll}): **사용자 실물(사진·단면·실측)을 그대로 옮긴 삼각 스케일**(2026-09).
  ...
  **종이에 닿는 두 모서리만** 치수를 잰다(`srEdges`; 능선은 떠 있어 못 쓴다). **스케일자로는 선을 긋지 않는다**(사용자) —
  ...
  숫자 위 **작은 숫자는 반대쪽에서 읽는 값**이다. 실물은 긴 눈금이 작은 숫자 한가운데를 지나지만 **숫자가 눈금에 닿지 않게**(사용자) 작은 숫자는 **제자리**(긴 눈금과 같은 줄, 큰 숫자 위 — 옆으로 비켰더니 "그대로 두라")에 두고 **긴 눈금을 그 앞에서 끝낸다**. 큰 숫자는 작은 숫자 아래(종이 쪽은 화면 mm 로 정해 면 좌표로 바꾼다). 눈금 표 `SRG` = [작은, 중간, 긴 중간, 숫자] —
  1/100 0.1·0.5·1, 1/200 0.1·0.5·1·5, 1/300~600 0.5·1·5·10(300~600 작은 눈금은 사진으로 확정 못 함). 1m(실제) = 종이 1000/S mm = 도면 denom/S m.
...
- **점 보이기는 도구 줄 '점' 체크**(`#cadShowPts`, 스냅 왼쪽 — 설정에서 옮겨 왔다). **설정(도구 줄 오른쪽 끝 톱니)**: **점 크기·번호 크기**(배율 30~300%, `SET.ptSize`·`ptNum` → `dotSvg`), **독립 점 제거**(`dropLonePoints`: 어떤 객체에도 쓰이지 않는 점을 확인 후 지우고 번호를 메운다 — 스케일자 마킹 점은 남긴다), 삼각자 크기(도면 m 환산 표시). **샤프 0.9·0.5 선 굵기**는
