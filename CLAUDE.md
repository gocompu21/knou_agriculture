...
- **스케일자**(도구 줄 '스케일자', `#cadSr`, localStorage `cadSr` = {x, y, rot, roll}): **사용자 실물(사진·단면·실측)을 그대로 옮긴 삼각 스케일**(2026-09).
  ...
  **종이에 닿는 밑면 폭(꼭짓점 사이)은 14.5mm**(사용자 실측). 위에서 본 반쪽 7.25mm = 종이 쪽 눈금 4.6 · 홈 0.8 · 능선 쪽 눈금 1.85mm(`SR_SEG`,
  ...
  구간마다 줄여 화면에 놓는다(글자도 그 비율로 세로를 누른다). **두 면이 만나는 모서리는 칼날이 아니라 눈금 없는 흰 평면(랜드)**이고 연필은 종이 쪽 랜드에 대고 긋는다 — 위에서도 흰 띠로 보인다(`SR_LAND` 0.45 · 능선 `SR_RIDGE` 0.4mm, 사용자 사진). 세 면 가운데 둥근 홈을 파 면마다 색칠 — 면 하나에 축척 둘: **빨강 1/100·1/200 · 청록 1/300·1/400 ·
