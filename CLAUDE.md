...
- **스케일자**(도구 줄 '스케일자', `#cadSr`, localStorage `cadSr` = {x, y, rot, roll}): **사용자 실물(사진·단면·실측)을 그대로 옮긴 삼각 스케일**(2026-09).
  ...
  **종이에 닿는 두 모서리만** 치수를 잰다(`srEdges`; 능선은 떠 있어 못 쓴다). **스케일자로는 선을 긋지 않는다**(사용자) —
  `srEdges` 는 `sr: true` 로 샤프 긋기 후보·점 붙기(`snapXY`)에서 빠지고, 템플릿 접선 맞춤에만 쓰인다. 모서리를 누르면 `srmark` 드래그가 되어 끌어도 선이 없다. 마킹으로 받는 모서리 곁은 `srMarkEdge` — 화면 8px 이면서 **자 두께의 22% 이하**(12px 고정이었을 때 자가 작게 보이면 몸통 전부가 마킹 범위라 샤프로 끌어도 안 옮겨졌다). 몸통 위에서는 십자 커서를 감춘다. **축척을 미리 고르지 않는다** — **자 오른쪽 여백 안의 축척 전환 단추**(`.sr-roll`, 크게 보이는 면의 여백 가운데, Shift 는 반대)를 누를 때마다 **크게 보이는 윗면(2/3)의 눈금이 1/100 → 1/200 → … → 1/600** 으로 바뀐다(`SR_CYCLE`: [바닥 면 `sr.roll`, 앞뒤 돌림 `sr.swap`] 여섯 상태). 굴리기만으로는 1/200·1/400·1/600 이 늘 좁은 아랫면에 있어 안 보였다(사용자). `sr.swap` 이면 판 전체를 180° 돌려 그리고(`srRot`) 능선을 1/3 자리로 옮겨(`srHA`) 크게 보이는 쪽이 화면 위에 오게 하며, 밝은·어두운 면·그늘·손잡이 자리도 함께 바꾼다. 도구 줄의 종류 고르기(`#cadSrKind`)에서 **1:N 사용자 지정**(예 1:430, `sr.custom`)도 고를 수 있다 — 네 눈금이 모두 1:N, 파란 홈, 굴림 손잡이 없음. 넣은 N 은 목록에 남는다(localStorage `cadSrCustom`). 눈금 간격은 `srGrad`(표에 없으면 종이 위 1mm·12mm 안팎). 사용자 지정 자의 눈금 길이는 `srRun` — 300mm 가 129m 처럼 끝수가 남으면 반대쪽 눈금이 비껴 서므로 숫자 간격의 배수(1:430 → 120m)로 끊는다.
  판 317mm, 눈금 300mm, **양 끝 여백 8.5mm**(실측은 12·13mm 였지만 "길다, 최대한 짧게" — 축척 표기가 겨우 들어가는 만큼). 표기 '1/300' 은 여백 가운데에서 0.8mm 바깥쪽, **작은 숫자 줄과 같은 높이**(`lb` = `sm`), 'm' 은 그 오른쪽 끝 위에 작게(0 눈금에 붙이면 답답하다는 지적). **양 끝 네 모서리는 둥글게**(반지름 2mm, `RR` 윤곽 + `srClip`) — 한 면의 두 축척은 서로 180° 돌린 모양이라 **한쪽 끝 눈금과 다른 쪽 0 이 같은 자리**다.
