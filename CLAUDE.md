...
- **스케일자**(도구 줄 '스케일자' + 면 고르기, `#cadSr`, localStorage `cadSr`): **사용자가 준 실물 삼각 스케일 사진 세 장을 재어 옮겼다**(2026-09).
  판 325×25mm, 눈금 300mm, 눈금 앞 여백 12mm · 뒤 여백 13mm(사용자 실측) — **윗면의 끝 눈금과 아랫면의 0 이 같은 자리**여야 한다(사용자 지적). **단면**(사용자 단면 사진)은 한 변 25mm 정삼각형(30cm 표준형), 세 변 가운데에 입구보다 큰 원으로 판 **둥근 홈**(입구 약 4mm)이 있고 그 안쪽이 색칠돼 있다 — 위에서 보면 가운데가 밝고 턱 밑 양쪽이 그늘진 원통 안쪽으로 그린다. 꼭짓점은 살짝 잘려 긴 변에 얇은 모서리 면(`SR_TIP`)이 보인다. 흔히 도는 '세 면이 통째로 오목한 3엽형' 설명은 이 자와 다르다 — 면은 평평하다. 한 번에 **한 면(두 축척)**이 보인다 — 윗면(눈금이 윗변, 0 이 왼쪽), 가운데 색 띠, 아랫면(눈금이 아랫변,
  글자가 거꾸로 서고 0 이 오른쪽). 면은 `SR_FACES`: **1/100·1/200 빨강 · 1/300·1/400 청록 · 1/500·1/600 검정**. `sr.s` 는 윗면 축척이고
  예전에 고른 1:200 등은 그 면으로 옮긴다. 사용자 지정 N 은 N · 2N 빨간 띠. 숫자 위 **작은 숫자는 반대쪽에서 읽는 값**(전체 − 눈금)이고
  숫자 자리의 긴 눈금이 그 한가운데를 지난다. 눈금 표 `SRG` = [작은, 중간, 긴 중간, 숫자] — 1/100 0.1·0.5·1, 1/200 0.1·0.5·1·5, 1/300~600 0.5·1·5·10
  (300~600 의 작은 눈금은 사진 해상도로 확정 못 해 0.5m 로 두었다). 1m(실제) = 종이 1000/S mm = 도면 denom/S m.
  **두 변 모두 자의 변**(`srEdges`)이라 샤프로 대고 긋고 접선 맞춤의 대상도 된다. 몸통을 끌면 0 눈금이 눈금에 붙고, 윗변 바깥 손잡이를
  **끌면 자유 회전**(정수°, Shift 15°), **누르기만 하면 다음 45° 자리로**. 몸통은 자처럼 가린다(`covered`·`onRuler`)
