...
- **도형 채우기**(도구 `fill`, 단축키 F, `fillAt`): 누른 면을 설정의 채우기 색·진하기(`SET.fillColor`·`fillOp`)로 칠한다.
  ...
  **채우기 유형**(설정 `SET.fillType` = solid·h·v·d(사선 ／)·x(X자 = 두 사선)·dot(점, 줄마다 반 칸 엇갈림, 반지름 = min(0.28U, 간격×0.13)) · gl(큰 가로세로 격자, 간격 1.5배) · gs(작은 격자, 간격 절반, 굵기 min(0.11U, 간격×0.16)) — X자는 선 굵기 절반): 수평선·수직선은 면 경로를 clipPath 로 삼아 테두리 상자(`op.bb`) 전체에
