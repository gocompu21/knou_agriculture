...
- **작도 기록·재생**(사용자: "삼각자·I바·스케일바의 움직임도 기록해 나중에 복귀해 볼 수 있게"): 선·원·글자를 넣을 때마다 그 순간의
  I자·삼각자·템플릿·스케일자 자리를 `op.tl` 에 적는다(`changed` → `stampTools`, `toolState`). 작도 순서 목록과 '작도 순서 복사'에
  "I자 Y30 · 45° 삼각자 X78 · 스케일자 1/300" 꼴로 붙는다(`tlText`). 기록 전에 그린 것은 `tl = 0`(afterLoad) — 재생 때 도구를 건드리지 않는다
