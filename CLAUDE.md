...
- **작도 기록·재생**(사용자: "삼각자·I바·스케일바의 움직임도 기록해 나중에 복귀해 볼 수 있게"): 선·원·글자를 넣을 때마다 그 순간의
  ...
  - **다음에 그을 자리 표시**(`rpMark`, 사용자 제안 — 노랑 투명 바탕 + 검은 테두리 동그라미, 테두리는 1.5px·불투명도 60% — 빼면 노란 삼각자 위에서 묻힌다): 한 칸 나아갈 때 시작점에
    ...
    (첫 점 번호 → 프리드로잉 첫 좌표 → 글자 좌표 → 칠한 면 가운데). **작도 순서의 줄을 눌러도 같은 동그라미가 뜬다**(사용자, 재생 밖에서도 — 단계 줄이면 그 단계의 첫 작도). 표식 토큰은 `rpMark.tok`. 이어서 **노란 형광펜**(`rpHilite`, 선 밑 층 `#cadHl`)이 시작점에서 끝점까지 그 선을 0.45초에 쓱 칠하고 잠시 머물다 사라진다 — 어느 선인지와 긋는 방향이 함께 보인다. 폭은 화면 14px, 쇄선도 이어진 띠로. 원·호는 둘레, 사각형은 네 변(쇄선이면 변마다), 글자·기호·칠한 면은 둘레 상자가 반짝(사용자 제안에 따라)
