...
- **제도판은 실물 A1 제도판 900×600mm 고정**(`BOARD_W`·`BOARD_H`, 사용자) — 종이를 무엇으로 고르든 판 크기는 같고 종이는 판 가운데에 실제 비율로 붙는다(종이 mm → 도면 m 는 축척). 레일·펜 받침 치수는 판 단위 `u`(6mm)로 센다. '맞춤'은 판 전체가 아니라 **종이 + 여백**을 잡는다(판을 다 잡으면 종이가 작아진다). 예전에는 판이 종이 크기에 비례했다.
...
- **스케일자**(도구 줄 '스케일자', `#cadSr`, localStorage `cadSr` = {x, y, rot, roll}): **사용자 실물(사진·단면·실측)을 그대로 옮긴 삼각 스케일**(2026-09).
  ...
  **종이에 닿는 두 모서리만** 치수를 잰다(`srEdges`; 능선은 떠 있어 못 쓴다). **스케일자로는 선을 긋지 않는다**(사용자) —
  ...
  바닥 면(`sr.roll`)을 바꾸면 종이에 닿는 눈금이 {1/100, 1/400}(검정 바닥, 처음) → {1/300, 1/600} → {1/500, 1/200} 로 바뀐다. 1:N 사용자 지정은 없앴다.
