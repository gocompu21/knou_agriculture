...
- **식물보호기사 필기 배지**(`_weed_gisa_counts`, `.ref.g` 보라색): 같은 자리에 붙되
  **낱개가 아니라 자격증별 건수**("기사 96 · 산업기사 85")다 — 방동사니 181건·바랭이
  ...
  글자 노드만 바꿔 원번호·배지·해설 마크업은 건드리지 않고, 한 글자 이름은 서버와
  같은 앞뒤 한글 규칙을 쓴다. 이미 칠한 곳은 건너뛰므로 '더 보기'로 이어 붙여도 겹치지 않는다
...
- **출제 범위 버튼**(`.wq-modes`, `api_weed_quiz_next?mode=`): 전체 / **기사-실기**(옛 '출제된
  것 우선' — `exam_count` 가 식물보호기사 실기 출제 횟수다) / **방송대** / **식보-필기** /
  **산기-필기** / 오답만. 뒤 셋은 그 출처의 문제·선지에 종명이 나온 카드만 내고
  문항 수로 가중한다(`_weed_source_weights`, 과목별 10분 캐시 — 카드 136장 × 기사 문항
  6,800건 대조). 방송대 13 · 식보 91 · 산기 84종 (별칭표로 새삼덩굴이 들어와 하나씩 늘었다)
