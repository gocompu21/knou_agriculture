...
- **스케일자**(도구 줄 '스케일자', `#cadSr`, localStorage `cadSr` = {x, y, rot, roll}): **사용자 실물(사진·단면·실측)을 그대로 옮긴 삼각 스케일**(2026-09).
  ...
  **종이에 닿는 두 모서리만** 치수를 잰다(`srEdges`; 능선은 떠 있어 못 쓴다). **스케일자로는 선을 긋지 않는다**(사용자) —
  ...
  판 317mm, 눈금 300mm, **양 끝 여백 8.5mm**(실측은 12·13mm 였지만 "길다, 최대한 짧게" — 축척 표기가 겨우 들어가는 만큼). 표기 '1/300' 은 **여백 가운데**(자 끝 ~ 0 눈금의 중간)에 조금 위로, 'm' 은 그 오른쪽 끝 위에 작게(0 눈금에 붙이면 답답하다는 지적). **양 끝 네 모서리는 둥글게**(반지름 2mm, `RR` 윤곽 + `srClip`) — 한 면의 두 축척은 서로 180° 돌린 모양이라 **한쪽 끝 눈금과 다른 쪽 0 이 같은 자리**다.
