...
- **스케일자**(도구 줄 '스케일자', `#cadSr`, localStorage `cadSr` = {x, y, rot, roll}): **사용자 실물(사진·단면·실측)을 그대로 옮긴 삼각 스케일**(2026-09).
  ...
  **종이에 닿는 밑면 폭(꼭짓점 사이)은 14.5mm**(사용자 실측). 위에서 본 반쪽 7.25mm = 종이 쪽 눈금 4.6 · 홈 0.8 · 능선 쪽 눈금 1.85mm(`SR_SEG`,
  ...
  구간마다 줄여 화면에 놓는다(글자도 그 비율로 세로를 누른다). **두 면이 만나는 모서리는 칼날이 아니라 눈금 없는 흰 평면(랜드)**이고 연필은 종이 쪽 랜드에 대고 긋는다 — 위에서도 흰 띠로 보인다(`SR_LAND` 0.45 · 능선 `SR_RIDGE` 0.4mm, 사용자 사진). 숫자는 **자리·눌림은 면 좌표 그대로(입체감)** 두고 굵기(종이 쪽 700·능선 600)와 색(#000)만 진하게, 같은 색 얇은 테두리(`paint-order: stroke`)로 획을 덮는다 — 한때 종이 쪽 글자를 눌리지 않게 폈다가 "이전처럼 입체감을 유지하되 숫자만 진하게"로 되돌렸다. 세 면 가운데 둥근 홈을 파 면마다 색칠 — 면 하나에 축척 둘: **빨강 1/100·1/200 · 청록 1/300·1/400 ·
