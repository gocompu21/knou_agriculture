...
| 탭 | 내용 |
|----|------|
| `textbook` 쪽집게 노트 | **주제별 교재** (`gisa/essay_textbook.py` → `build_textbook`, 10분 캐시). 분류(topic_group) pill → 주제(topic_key) 아코디언. 대상: 2회 이상 출제 + 계산 + 재출제 유력(자연생태복원 193주제, 식물보호 51주제). 별 = 출제 회차 수(자연생태복원 5회↑ ★★★ / 3~4 ★★ / 2 ★ / 1 ☆), 회차 배지(`note_badges`), 정리 자료(`freq58` 정의·이유·사례, `calc` 공식·대입·함정)를 대표 문항 → topic_key 로 이어 붙이고, 정리가 없으면 최근 문항+모범답안. 걸러 보기(★★★/계산/재출제/⚠️), 기출 문항 펼침(`siblings?all=1`) |
