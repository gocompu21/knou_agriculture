...
- **두 줄(경계석)**(`#cadDbl`, `makeDouble`, 사용자) — 고른 선에 간격(설정 '두 줄 간격', 기본 0.2m)을 두고 한 줄 더. 닫힌 것(사각형·원·끝과 끝이 이어져 닫힌 선들·닫힌 프리드로잉)은 **안쪽**(면적 부호로 안쪽을 가린다), 열린 것은 쪽을 고른다(첫 변이 가로면 위/아래, 세로면 왼쪽/오른쪽, 호는 안쪽/바깥쪽). 이어진 선들은 한 꺾은선으로 모아 변마다 평행이동한 뒤 이웃 변의 교점을 새 꼭짓점으로 — 모서리가 맞물린다(`offsetChain`). 새 선은 원래 선의 선 종류·굵기, 여러 변이면 '경계석 안쪽선' 묶음
