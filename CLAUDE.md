...
- **글자 도구는 도면을 누르면 그 자리에 입력 상자**(`openTextBox`)가 뜨고 Enter 로 넣는다(Esc 그만두기, 다른 곳을 누르면
  쓴 것을 넣는다). **글자는 점을 찍지 않고 좌표(`op.x`·`op.y`)를 직접 갖는다** — 누른 자리가 글의 시작이라 가로는
  오른쪽으로(`text-anchor=start`), 세로(`op.dir='v'`, `writing-mode:vertical-rl`)는 아래로 써진다. 옛 글자(`op.p`)는 점을 쓰고 가운데
