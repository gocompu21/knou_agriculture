...
- **작도 기록·재생**(사용자: "삼각자·I바·스케일바의 움직임도 기록해 나중에 복귀해 볼 수 있게"): 선·원·글자를 넣을 때마다 그 순간의
  ...
  - 작도 순서 칸의 **▶ 재생**(`rpStart`) → **작도 순서 탭 위** 조작 막대(`#cadRp`, 사용자 — 도면 위에 띄우면 종이를 가렸다; 재생을 켜면 작도 순서 탭으로 바꾼다): ⏮ ◀ ▶재생/멈춤 ▶ ⏭ · 빠르기 · 끌어 옮기는 막대(앞뒤로 끌면 그 칸까지 그려진 모습 — 핸들러에서 값을 먼저 읽어야 한다, rpStop 의 rpInfo 가 막대를 되돌린다) · 닫기.
    ...
    (`rpReveal` — rAF 로 dashoffset 을 늘리며 **샤프 그림이 선 끝을 따라간다**: 커서로 쓰는 실사 샤프 PNG 를 스타일시트의 `.cad.cad-tool-pen9/5 #cadSvg` cursor 에서 읽어 `#cadRpPen` 에 화면 px 크기로 놓고 샤프심 끝(hotspot)을 선 끝에 맞춘다, 샤프 0.5 선이면 0.5 그림. 칠하기·글자는 샤프 없이 서서히 나타난다). 지금 선은 빨강(`g.rp-cur`), 목록의 그 줄은 노랑(`.cad-op.rp`)
