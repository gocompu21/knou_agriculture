...
- **도형 채우기**(도구 `fill`, 단축키 F, `fillAt`): 누른 면을 설정의 채우기 색·진하기(`SET.fillColor`·`fillOp`)로 칠한다.
  ...
  **채우기 유형**(설정 `SET.fillType` = solid·h·v·d(사선 ／)·x(X자 = 두 사선)·dot(점, 줄마다 반 칸 엇갈림, 반지름 = min(0.28U, 간격×0.13)) · gl(큰 가로세로 격자, 간격 1.5배) · gs(작은 격자, 간격 절반) — X자·작은 격자는 선 굵기 절반): 수평선·수직선은 면 경로를 clipPath 로 삼아 테두리 상자(`op.bb`) 전체에
  평행선을 긋고 오려 낸다. 간격(`op.gap`)은 만들 때 면의 짧은 쪽 ÷ 8 을 0.1·0.2·0.25·0.5·1·2·2.5·5 … 로 반올림 — 면이 커지면 넓어진다.
  선은 채우기 색, 진하기는 최소 0.85(옅으면 안 보였다). 굵기는 0.14U 와 **간격의 1/5** 중 작은 값(X자는 두 겹이라 그 절반 — 0.07U·간격의 1/10) — 좁은 면에서 선이 뭉쳐 면처럼 보였다
