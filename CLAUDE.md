...
- **스케일자**(도구 줄 '스케일자' + 축척 고르기 1:100~1:1000 · **사용자 지정**(1:N 을 물어 목록에 더한다, localStorage `cadSrCustom`), `#cadSr`, localStorage `cadSr`): 삼각 스케일의 한 면 — 길이 31cm·폭 24mm(종이 위),
  윗변에 고른 축척 눈금, 가운데 붉은 띠는 **V 홈처럼 파인 입체**(윗단 그늘·아랫단 빛 턱), 몸통은 **불투명**(위 밝고 아래로 어두워지는 그라데이션 + 바닥 그림자).
  그라데이션은 `gradientUnits=userSpaceOnUse` 로 srMap 두 점에 걸어 돌려도 음영이 자를 따라 돈다. **눈금 1m(실제) = 종이 1000/S mm = 도면 denom/S m**(1:430 종이에 1:500 자면 0.86m). 눈금은 **실제 삼각
  스케일을 따른 표**(`SRG` — 축척별 [작은, 중간, 숫자] 간격 m: 1:100 [0.1,0.5,1] · 1:200 [0.2,1,2] · 1:250~400 [0.5,2.5,5] · 1:500~1000 [1,5,10]),
  첫 숫자에 단위(1m·5m·10m). 사용자 지정 축척은 종이 위 1mm·12mm 안팎으로 고른다. 템플릿처럼 `srMap`·`srLocal`(회전 포함), 몸통을 끌면 **0 눈금이 눈금에 붙고**,
  윗변 바깥 손잡이를 **끌면 자유 회전**(정수°, Shift 15°), **누르기만 하면 다음 45° 자리로**. **눈금 변은 자의 변**이라 `guideEdges` 에 들어가 샤프로 대고 긋고(몸통 위 오른쪽 끌기도),
  접선 맞춤의 대상도 된다. 몸통은 자처럼 가린다(`covered`·`onRuler`)
