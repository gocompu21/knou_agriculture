...
- **자가 덮은 곳은 지워지지 않는다**(`covered` = I자 몸통·두 날 + 삼각자 아크릴, 가운데 구멍은 종이라 제외).
...
- **삼각자 가운데 투명 회전 아이콘**(`.tri-rot`, 구멍 안): 누르면 시계방향 90°(`rotateTri`). 실물처럼 다시 I자에 얹힌다 —
...
- **삼각자 45° · 60°**(도구 줄 단추, 켜고 끄는 것 — 도구가 아니다): I자 윗날에 얹혀 좌우로만 민다(I자를
  ...
  아크릴(처음 주황이었다가 '투명으로' 요청) + 가운데 구멍(evenodd — 구멍으로는 밑의 종이를 누른다). 몸통을 끌지 않고 누르면 통과한다(투명).
