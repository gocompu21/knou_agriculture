...
- **두 줄(경계석)**(`#cadDbl`, `makeDouble`, 사용자) — 고른 선에 간격을 두고 한 줄 더 — **간격은 누를 때마다 뜨는 작은 창에서 사용자가 정한다**(기본은 마지막에 쓴 값, 처음 0.2m; Enter 로 긋기). 닫힌 것(사각형·원·끝과 끝이 이어져 닫힌 선들·닫힌 프리드로잉)은 **안쪽**(면적 부호로 안쪽을 가린다), 열린 것은 쪽을 고른다(첫 변이 가로면 위/아래, 세로면 왼쪽/오른쪽, 호는 안쪽/바깥쪽). **선과 호를 끝과 끝으로 이어 한 사슬(고리)로 모은다**(`dblTargets` — 끝점 번호가 달라도 0.002m 안이면 이어진 것; 둥근 모서리가 낀 윤곽을 따로 처리했더니 안과 밖이 섞였다, 사용자). 고리의 안쪽은 면적 부호로 한 번에 정하고(호는 점을 뿌려 넣는다), 선은 법선으로·호는 반지름을 늘이거나 줄여 평행이동, 이음매는 선끼리면 교점·호가 끼면 호 끝점. 열린 사슬은 첫 토막 기준으로 고른 쪽을 사슬 전체에. 새 선은 원래 선의 선 종류·굵기. **새 선마다 손으로 그었다면 쓸 도구 자리를 계산해 `tl` 에 적고**(`toolsForLine` — 가로선은 I자 윗날을 그 높이에, 세로선은 I자를 선 아래 끝에 두고 삼각자 세로 변을 그 x 에, 45°·60°·30° 선은 그 삼각자 빗변을 선에 맞춘다; 다른 각도는 지금 자리; **호·원은 `toolsForCircle` — 지름이 가장 가까운 원 구멍의 중심을 호의 중심에 맞춰 템플릿을 놓는다**, 종이 위 1mm 넘게 어긋나면 지금 자리), **만든 즉시 그 부분을 재생해 보인다**(`RP.autoEnd` — 끝나면 도구는 재생 전 자리로). 변마다 도구가 옮겨 가야 해서 묶지 않는다(템플릿 도형과 다르다)
