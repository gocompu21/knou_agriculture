...
- **I자 날은 위아래 둘이다**(실물처럼). 폭(`barW`)은 눈금의 배수라 윗날이 눈금 위면 아랫날도 눈금 위다
  (401 은 4m). 긋기·붙기는 가까운 날로 간다(`nearEdge`). 표시 'Y 30 · 34'
...
- **삼각자 45° · 60°**(도구 줄 단추, 켜고 끄는 것 — 도구가 아니다): I자 윗날에 얹혀 좌우로만 민다(I자를
  옮기면 따라온다). 손으로 끌어 밀고, '뒤집기'(빗변 좌우), 60° 자는 '빗변 30°'(눕히기)로 바꾼다. 반투명
  아크릴 + 가운데 구멍(evenodd — 구멍으로는 밑의 종이를 누른다). 몸통을 끌지 않고 누르면 통과한다(투명).
