...
- **제도판**(`#cadBoard`, `boardSvg`·`boardGeom`): 사용자가 보여 준 탁상 제도판을 평면으로 종이 둘레에 그린다 — 흰 판(그림자),
  ...
  제도판 폭에 맞췄고, '맞춤'도 제도판 전체가 들어오게 잡는다. 사용자가 준 평행자 사진을 따라 **I자는 밝은 은색 몸통 + 윗변의
  가는 짙은 띠**, 양 끝은 레일 위에 앉은 **회색 죔쇠 + 톱니 테두리 조임 다이얼**, 바깥으로 **튀어나온 레버**(끝이 둥근 손잡이), 레일 바깥 모서리를 거는 **갈고리**로 그린다.
  왼쪽 레일 안쪽에 짙은 눈금 띠, 레일 아래쪽 검은 멈추개, 펜 받침은 나사 둘 달린 짙은 홈 + 그 아래 밝은 틀. 판은 `pointer-events:none` 이라 누름을 가로채지 않는다
