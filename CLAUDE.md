...
- **제도판은 도면(종이) 크기에 맞춘다**(사용자: "A1 으로 고정하지 말 것") — 종이 둘레에 여백(좌우 7U·위 6.5U·아래 8.5U)을 붙인 크기이고, I자는 판의 좌우 레일에 걸리므로 판을 따라 맞춰진다. 한때 900×600mm 고정이었다가 되돌렸다. **I자는 판 위 틀 밑 ~ 펜 받침 위까지 오르내린다**(`setBarY`). '맞춤'은 판 전체를 잡는다. 스케일자·삼각자·템플릿은 실물 mm 를 축척으로 바꾼 크기다
