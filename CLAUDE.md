...
- **제도판은 도면(종이) 크기에 맞춘다**(사용자: "A1 으로 고정하지 말 것") — 종이 둘레에 여백(좌우 7U·위 6.5U·아래 = I자 폭 + 6U, 최소 8.5U)을 붙인 크기이고 — 아래 여백이 좁으면 I자 윗날이 종이 맨 아래까지 못 내려갔다(사용자: "끝까지 내릴 수 있게"), I자는 판의 좌우 레일에 걸리므로 판을 따라 맞춰진다. 한때 900×600mm 고정이었다가 되돌렸다. **I자는 판 위 틀 밑 ~ 펜 받침 위까지 오르내린다**(`setBarY`). '맞춤'은 판 전체를 잡되 **위에 붙인다** — 화면이 세로로 남으면 남는 자리를 아래로 몬다(가운데 맞춤은 위 여백이 넓어 보였다, 사용자). 위 여백은 1.2U, 좌우·아래 3.4U. 스케일자·템플릿은 실물 mm 를 축척으로 바꾼 크기, 삼각자는 종이 높이 비율이다
- **제도판**(`#cadBoard`, `boardSvg`·`boardGeom`): 사용자가 보여 준 탁상 제도판을 평면으로 종이 둘레에 그린다 — 흰 판(그림자),
  ...
  빼고 다이얼만 실사로' 요청). 왼쪽 레일 안쪽에 짙은 눈금 띠, 레일 아래쪽 검은 멈추개. **아래 끝은 펜 받침 하나**(나사 둘 달린 짙은 홈)다 —
  그 아래 밝은 틀과 아래 모서리 마개는 뺐다. 판은 `pointer-events:none` 이라 누름을 가로채지 않는다
...
- **삼각자 크기는 종이 높이에 대한 비율**(사용자: "스케일자·템플릿은 그대로, 삼각자는 종이 크기에 맞게") — 직각을 낀 긴 변 = 종이 높이 × 설정 %(45° 기본 50% · 60° 기본 70%, `SET.triPct45`·`triPct60`). 설정 칸 옆에 도면 m 와 종이 위 cm 를 함께 보인다. **45° 자는 직각변을 굵은 눈금(5m) 배수로 맞춘다** — 꼭짓점이 눈금 교차점에 오게. 한때 실물 cm(45° 12cm · 60° 45cm)를 축척으로 바꿨는데 종이가 작으면 자가 종이를 넘었다
