...
- **교수 메모**(`.wq-notes-ov`): 답을 고르면 **문제 사진 왼쪽 아래에 흰 글씨**로 얹힌다
  ...
  **밝은 사진**이 있어 글자 그림자만으로는 안 읽히므로, 글 뒤에만 옅은 어둠을 까는
  그라데이션을 함께 쓴다. 이 그라데이션이 모서리 밖으로 새지 않도록 `.wq-photo-in` 에
  `border-radius: 8px; overflow: hidden`. 긴 메모가 손그림을 침범하지 않도록 폭은 58%,
  손그림에 `z-index: 1`
...
- **답안 수정**(스태프): 답 아래 "답안 수정" 버튼 → 식별 포인트·헷갈리는 종과 구별·
  ...
  **Toast UI 는 마크다운 기반이라 글 색을 못 넣으므로 여기서는 쓰지 않는다** —
  ...
  - **교수 메모는 서식을 받지 않는다** — 사진 위 흰 글씨라 색이 안 보인다. 줄 단위
    필드라 `<li>`·`<br>` 을 줄바꿈으로 되돌려 저장하고, 편집기도 툴바 없이 보여 준다
