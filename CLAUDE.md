...
둘이면 3.33점. 부분 점수도 비율로 반영된다.

요구한 개수를 채웠으면 못 쓴 보기에 `alt` 를 달아 화면이 **빨간 ✗ 가 아니라 회색
점**으로 그린다(`.rs-pt.alt`). ✗ 로 두면 만점인데도 감점당한 것처럼 읽힌다.
같은 까닭으로 **만점이면 `missing`('↳ 빠짐: …')을 지운다.**

실측(2025-2회 4번): 셋 맞힘 5.0/5.0 · 둘 3.33 · 한해 대책(6항목 중 3) 5.0/5.0.
합성 문항은 그대로다 — 환경영향평가(정의+5항목)에서 정의를 빼면 3.35/4.0,
여울과 소(차이 2가지+영향)에서 영향을 빼면 1.76/3.5.
