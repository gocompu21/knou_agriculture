...
**단, 덧그리기가 쌓인 뒤에는 `xhigh` 가 듣는다**(2026-09, 401 입체평면도 14~16회차).
한 그림을 열몇 번 고쳐 그리면 **회차마다 조금씩 물러진다** — 라플라시안 분산이
1125 → 916 으로 19% 떨어졌다. "어느 부분도 앞 회차보다 흐려지면 안 된다"를 프롬프트에
박아도 942 로 거의 회복되지 않았다. **열화는 프롬프트로 못 막는다.** `high` → `xhigh`
(장당 $0.041 → $0.074)로 올리니 1015 로 돌아왔다. 새 그림에는 `high` 로 충분하고,
**여러 회차 덧그린 그림을 마무리할 때만** 올린다.

> 지표가 10% 낮아도 **바뀌지 않은 영역을 2배로 확대해 1:1 로 견주는 것**을 잊지 말 것.
> 위 사례에서 지붕 너와·난간 살·데크 널 이음매는 두 그림이 구분되지 않았다 —
> 줄어든 것은 잔디·수관의 고주파 잡티였지 디테일이 아니었다.
