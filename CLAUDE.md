...
- **I자(평행자)**: 제도판의 가로자. 켜면 종이 폭 전체에 눈금 자가 놓이고, 끌거나 ↑↓(Shift 는
  5칸) 로 위아래로 옮긴다. **윗변 16px 안을 누른 점은 그 높이(barY)에 붙는다**(`snapXY`) — 수평선을
  긋는 도구다. 자 몸통을 끌지 않고 그냥 누르면 평소 누름으로 넘긴다(자 위에서도 점을 찍을 수 있게).
