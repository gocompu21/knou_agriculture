...
- **스케일자**(도구 줄 '스케일자', `#cadSr`, localStorage `cadSr` = {x, y, rot, roll}): **사용자 실물(사진·단면·실측)을 그대로 옮긴 삼각 스케일**(2026-09).
  ...
  **종이에 닿는 두 모서리만** 치수를 잰다(`srEdges`; 능선은 떠 있어 못 쓴다). **스케일자로는 선을 긋지 않는다**(사용자) —
  ...
  바닥 면(`sr.roll`)을 바꾸면 종이에 닿는 눈금이 {1/100, 1/400}(검정 바닥, 처음) → {1/300, 1/600} → {1/500, 1/200} 로 바뀐다. 도구 줄의 종류 고르기(`#cadSrKind`)에서 **1:N 사용자 지정**(예 1:430, `sr.custom`)도 고를 수 있다 — 네 눈금이 모두 1:N, 파란 홈, 굴림 손잡이 없음. 넣은 N 은 목록에 남는다(localStorage `cadSrCustom`). 눈금 간격은 `srGrad`(표에 없으면 종이 위 1mm·12mm 안팎).
