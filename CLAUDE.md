...
- **템플릿으로 한 번에 그린 다각형은 한 묶음**(`op.grp`·`gname`, 사용자 — 변마다 따로 포커스를 받았다) — 변은 여전히 선 줄들이라 지우개·접선·채우기는 그대로 쓰고, 작도 순서·작도 순서 복사·재생·형광펜·지우기·↑↓·끌기·⇄ 가 묶음을 한 줄로 다룬다(`grpSpan`·`grpOps`·`grpDesc`: '정삼각형 P1 → P2 → P3 → P1 · 한 변 12m'). 재생은 묶음 가운데에서 멈추지 않고 변을 차례로 긋는다. 원은 원래 한 줄
