...
- **2026-1회 16문항**은 복원본 두 벌을 대조해 넣었다(`load_eco_2026_1.py` — 자격증 이름·연도·회차·번호로
  찾는 멱등 스크립트라 서버에서 그대로 돌린다. 배점은 유형별 기본값을 45점에 맞추고, topic_key 는 같은
  주제의 앞선 문항에서 물려받는다). 복원본끼리 어긋난 곳은 `notes` 에 적었다
...
**답을 쓴 문항은 모두 LLM이 채점한다.** 반환 형식은 어느 경로든 같다.

| 유형 | 엔진 | 방식 |
|------|------|------|
| 빈 답안 | `rule` | 호출 없이 0점. 채점이 아니라 처리다 |
| 계산 | `rule` → `llm-calc` | 최종 답 수치가 전부 맞으면 규칙으로 만점. 어긋나면 계산 전용 LLM |
| **그 밖의 전부** | `llm` | 채점 기준표의 포인트마다 "답안이 이 내용을 담았는가" 판정 |

**프롬프트는 종목을 알려 준다**(`_exam_header`). "국가기술자격 실기 필답형"이라고만
하면 모델이 어느 분야인지 모른 채 용어를 판정한다. 문항의 자격증과 `EXAM_INFO` 의
`practical_subject` 를 합쳐 `[시험] 식물보호기사 실기 필답형 · 과목 「식물보호실무」 ·
필답 100점 만점` 을 프롬프트 맨 위에 붙인다 — '엽면적지수'와 'LAI', '무황산근'과
'황산근이 없는 비료'가 같은 말인지는 그 분야를 아는 채점위원이라야 가른다.
계산형 프롬프트도 같은 머리말을 쓴다.

**배점을 주고 점수를 매기게 한다**(2026-09). 프롬프트의 채점 기준표에 항목별 배점이
적혀 있고(`1. (2.5점) ① 최적엽면적`), 모델이 항목마다 `matched` 와 **`score`(그 항목에
주는 점수)** 를 돌려준다. 부분 점수도 여기서 나온다 — "배점 2.5점짜리를 반만 맞혔으면
1.25". 예전에는 "부분적으로만 맞으면 matched=false" 라 항목 단위로 전부 아니면
전무였고, 한 가지만 묻는 서술형처럼 **기준표가 한 항목뿐인 문항은 0점 아니면 만점**만
나왔다. 단답·빈칸은 배점 전부 또는 0만 주도록 못박았다 — 맞거나 틀리거나 둘뿐이다.
계산형도 같아서 `answer_score`(배점의 70% 이내)와 `process_score`(30% 이내)를 받는다.

> **코드가 하는 일은 자르기와 더하기뿐이다.** 채점 판단은 전부 모델이 하되,
> 항목 점수가 배점을 넘거나 음수면 잘라 내고(9.9 → 2.5, -3.0 → 0), 합계는
> 파이썬이 낸다 — 덧셈은 모델이 틀릴 이유가 없는 대신 틀리면 점수가 어긋난다.
> `matched=true` 인데 `score` 를 0 으로 보내면 판정을 따라 배점 전부를 준다.
>
> 화면은 부분 인정을 **△ '부분 인정 1.25/2.5점'** 으로 그린다
> (`.pt.half`·`.rs-pt.half`). ✕ 로 두면 점수가 붙은 까닭을 알 수 없다.

한때 단답·빈칸을 문자열 비교로 채점했으나 **걷어냈다**(2026-09-01 `082da86`).
모범답안이 "야생절멸(EW)"인데 "야생절멸"만 쓴 정답을 0점 처리하는 사고가 났다.
동의어·표기 변형·괄호 부연·띄어쓰기 같은 예외는 규칙으로 끝이 없고, **틀리게 일관된
채점은 사용자가 채점 자체를 불신하게 만든다.** 아끼는 비용은 세션당 1원 미만이라
정확도와 바꿀 값어치가 없다. 관용 규칙은 대신 프롬프트에 적어 두었다(괄호 부연은
선택 사항 / 띄어쓰기·맞춤법 무시 / 빈칸 번호기호 무시).

> **계산형의 규칙 경로(수치만 맞으면 만점)도 걷어냈다**(2026-09-15). 조경
> 학원예상 1회 7번에서 `42.67 m2/hr`(정답 m³/hr)에 만점을 줬다 — 숫자만 보고
> **단위를 못 봤고**, 허용 오차 2% 때문에 `3.2 x 0.8` 을 쓴 답안이 중간값 2.56 과
> 맞은 것으로 잡혔다. 이제 답을 쓴 계산형은 모두 `llm-calc` 다. 배점의 70%를 답
> 정확성, 30%를 과정에 배분한다(답이 맞으면 과정을 생략해도 70%는 준다).
>
> **단위 규칙은 `CALC_SYSTEM` 에 있다** — 실기 답안 작성 유의사항 그대로다.
> 단위가 틀리면(m²/hr ↔ m³/hr) 수치가 맞아도 답 점수 0(과정 점수는 줄 수 있음),
> 단위를 빠뜨렸을 때는 문제가 단위를 지정했으면(`작업량(m³/hr)을 구하시오`) 정답,
> 지정하지 않았으면 오답. `m3`·`m2` 같은 위첨자 없는 표기는 같은 단위로 본다.
> 실측: m2/hr 0.9/3 · m3/hr 3/3 · 단위 생략 3/3.

만점 보정: 균등 배분(4점 ÷ 3항목 = 1.33)에서 생기는 반올림 오차 때문에 전부 맞아도
3.99점이 나온다. 모든 포인트가 matched면 만점으로 올린다.

#### '3가지를 쓰시오'인데 모범답안이 5가지일 때 — 셋만 맞혀도 만점 (2026-09)

"토양 공기를 지배하는 요인을 **3가지** 쓰시오"의 모범답안은 토성·토양구조·경운·
토양수분·유기물 **다섯**이다. 이 다섯은 '다 써야 할 것'이 아니라 **인정되는 답의
목록**이므로, 셋을 맞혔으면 만점이다. 그런데 기준표를 5항목 × 1점으로 쪼개 놓고
더하기만 하니 3점이 나왔다(식물보호기사 2025-2회 4번). 보기 목록형 문항은
로컬 기준 **43건**이다.

**보기 묶음은 모델이 짚고, 코드는 관문만 지킨다.** 규칙으로 가르려 하면
'정의를 쓰고 3가지를 설명하시오' 같은 합성 문항까지 만점이 된다 — 그 판단은
문제를 읽어야 나온다. 그래서 채점 응답에 두 칸을 더 받는다:

| 필드 | 내용 |
|------|------|
| `pick_count` | 문제가 요구한 개수. 보기 목록이 아니면 0 |
| `pick_indices` | 서로 바꿔 써도 되는 기준표 항목 번호. **정의·개념처럼 반드시 써야 하는 항목은 넣지 않는다** |

코드(`_pick_group`)는 모델 말을 그대로 믿지 않고 세 가지를 본다 —
① `pick_indices` 가 `pick_count` 보다 많아야 한다(같으면 그냥 다 써야 하는 문항이다),
② **그 개수가 문제문에 실제로 적혀 있어야 한다**(`asked_count`), ③ 문제문의 '가지'
세는 자리가 **하나뿐**이어야 하고 '각각'·'씩'이 없어야 한다.

> ③이 합성 문항을 걸러낸다. 'A를 2가지와 B를 3가지'는 자리가 둘이라 다섯을 다
> 써야 하는 문항이고, '논과 밭에 각각 1가지씩'은 대상마다 따로 요구한 것이다.
> 정의+N가지 꼴은 ①이 막는다 — 보기 묶음이 딱 N개라 `len(idx) > need` 가 안 된다.
> `asked_count` 의 한글 수사 앞에는 `(?<![가-힣])` 를 둔다. 없으면 '다양**한** 가지'의
> '한'을 1로 센다.

점수는 **보기 묶음의 배점 전부를 요구한 개수로 나눠** 준다(`_sum_points`). 다섯
항목에 1점씩 붙어 있어도 셋만 요구했으면 하나가 1.67점이다 — 셋 맞히면 5점,
둘이면 3.33점. 부분 점수도 비율로 반영된다. **모범답안이 10가지든 20가지든
분모는 요구 개수**라 셋만 맞히면 만점이다.

**요구한 개수보다 많이 쓰면 먼저 쓴 것부터 그 개수까지만 센다.** 실기 채점은
더 쓴다고 봐주지 않는다 — 셋을 요구했는데 넷을 적었다면 앞의 셋으로 판단하지,
맞은 것만 골라 주지 않는다. 그래서 채점 응답에 `answer_order`(수험자가 쓴
차례대로 그 항목의 기준표 번호, 기준표에 없는 답은 0)를 함께 받아 앞의 N개만
점수에 넣는다(`_written_order`). 넘겨 쓴 항목에는 `over` 를 달아 화면이 회색으로
"3가지를 넘겨 쓴 항목입니다"라고 밝힌다 — 맞은 항목이 ✓ 인데 점수가 없으면
왜인지 알 수 없다.

> **차례는 기준표가 통째로 보기 묶음일 때만 본다.** '정의를 쓰고 3가지'처럼
> 반드시 써야 하는 항목이 섞이면 쓴 차례 가운데 어느 것이 보기인지 가릴 수
> 없다(정의가 첫 칸을 먹어 보기 하나가 잘려 나간다). 점수를 준 항목이 차례에
> 빠져 있어도 모델이 대충 적은 것이므로 버리고 '잘 맞힌 것부터' 방식으로 돌아간다.

요구한 개수를 채웠으면 못 쓴 보기에 `alt` 를 달아 화면이 **빨간 ✗ 가 아니라 회색
점**으로 그린다(`.rs-pt.alt`). ✗ 로 두면 만점인데도 감점당한 것처럼 읽힌다.
같은 까닭으로 **만점이면 `missing`('↳ 빠짐: …')을 지운다.**

실측(2025-2회 4번, 요구 3 · 모범답안 5):

| 답안 | 점수 |
|------|------|
| 셋, 모두 정답 | 5.0 / 5.0 |
| 넷, 앞의 셋이 정답 (넷째는 `over`) | 5.0 / 5.0 |
| 넷, 첫째가 오답 | 3.33 |
| 다섯, 첫 둘이 오답 | 1.67 |
| 둘만 씀 | 3.33 |

한해 대책(6항목 중 3)도 5.0/5.0. 합성 문항은 그대로다 — 환경영향평가(정의+5항목)에서
정의를 빼면 3.35/4.0, 여울과 소(차이 2가지+영향)에서 영향을 빼면 1.76/3.5.
...
비용 때문에 flash 로 옮길 이유는 없다. 2026-09 에 같은 시험지 1쪽(입력 2,951토큰:
사진 1,100 + 프롬프트 1,851)을 재 보니 **pro 약 11원**($0.0080, 생각 토큰 없음),
**3.8-flash 약 7원**($0.0048, 출력 161 + 생각 531 이 출력 단가로 청구)이었고, flash 는
2027-01-01 부터 단가가 두 배라 **pro 보다 비싸진다**(약 13원). 한 번 flash 로 바꿨다가
이 측정을 보고 되돌렸다.

> **preview 모델은 쓰지 않는다** — 예고 없이 종료된다. 2026-09 에 판독을 뺀 전 용도를
> `gemini-3.8-flash` 로 맞췄다. 판독만 정확도 때문에 preview 를 감수한다. 모두 환경변수로 교체 가능.
...
| `/gisa/<id>/essay/pass/` | `essay_pass` | `essay_pass.html` | **합격 전략** — 회원용 글. 문서를 읽어 렌더한다 |
...
#### 합격 전략(`essay_pass`) — 글은 문서 한 벌만 둔다 (2026-09)

회원에게 "무엇부터 공부할지"를 알려 주는 글이다. 실기 페이지 히어로와 쪽집게 노트
머리글에 **혼자 노란 버튼(`★ 합격 전략`)**으로 붙어 있다 — 다른 링크는 흰색·반투명인데
처음 온 사람이 가장 먼저 보아야 할 글이라 색을 달리했다.

- **본문은 `docs/실기합격전략.md` 를 읽어 렌더한다.** 템플릿에 글을 적어 두면 두 벌이
  되어, 회차가 늘어 재출제율을 고칠 때 한쪽만 고쳐 어긋난다. 문서 머리말(작성 경위·
  갱신 방법)은 회원에게 보일 것이 아니므로 **첫 `---` 앞을 잘라 낸다**
- **문서가 있는 자격증에만 연다**(`PASS_DOCS`). 본문에 그 종목의 수치가 박혀 있다 —
  재출제율·분야 비중·"잡초방제학은 건너뛰어도 된다". 학습전략을 `freq58` 노트로 거르는
  것과 같은 까닭이며, 없는 자격증은 뷰가 목록으로 돌려보내고 링크도 내지 않는다
- 식물보호 두 급수와 **자연생태복원기사**(`docs/자연생태복원_실기합격전략.md`)가 있다.
  다른 종목에 붙이려면 **그 종목 수치로 글을 새로 써서** `docs/` 에 두고 `PASS_DOCS`
  에 등록한다 — 남의 수치를 그대로 보이면 안 된다
- **결론이 종목마다 반대일 수 있다.** 식물보호는 기출이 18장뿐이라 재출제율이 높아
  "빈출을 좁게 파라"가 맞지만, 자연생태복원은 51회차에 서로 다른 주제가 469개라
  **좁히면 무너진다** — 기출 주제를 전부 훑으면 최근 8회차 평균 29.3/45점인데,
  2회 이상 반복된 주제만 보면 14.4점, 3회 이상만 보면 6.6점이다. 글을 옮겨 쓰지
  말고 그 종목 데이터로 다시 세야 하는 까닭이다
  - 세는 법: 회차마다 "그 문항의 `topic_key` 가 **앞선 회차에** 있었나"를 보고
    `points` 로 가중해 만점으로 환산한다. 회차를 더하면 문서의 수치부터 고칠 것
- 수치의 근거와 갱신 방법은 문서 머리말에 적어 두었다(기출 22개 회차 438문항 기준)

**필기와 실기는 목록에서부터 완전히 분리한다.** 자격증 목록(`/gisa/`)은 **'필기'와
'실기' 두 묶음**으로 나뉘고(`written`·`practical`, 묶음 안은 자격증 이름순), 한
자격증이 양쪽에 한 장씩 들어간다. 실기 카드는 `essay_list`로 직행한다.

**카드 색이 그 다음 화면의 색이다.** 필기 카드는 남색(`#f3f5ff` 바탕·`#3949ab` 띠),
실기 카드는 초록(`#e9f4ec` 바탕·`#2d6a4f` 띠) — 필기 상세·기출고사가 남색이고 실기
화면이 다크그린이라, 목록에서 고른 색이 이어진다. 이름·배지도 같은 계열로 간다.

> 실기 쪽 초록 바탕을 **흰빛으로 옅게 깔면 안 된다.** `base.html` 의 페이지 바탕이
> 연녹 그라데이션이라 `#f7fbf8` 정도는 그 위에서 묻혀 카드 경계가 사라진다.
> 한 단계 진한 `#e9f4ec` 라야 떠 보인다(필기는 남색 계열이라 이 문제가 없다).

> 처음에는 한 그리드에 섞어 놓고 카드마다 `[필기]`·`[실기]` 배지로만 갈랐는데,
> 아홉 장이 줄줄이 늘어서 어느 것이 무엇인지 한눈에 들어오지 않았다. 묶음을
> 나누니 배지가 필요 없어져 뺐고(머리말이 이미 말한다), "필답형 주관식 · 인쇄해서
> …" 설명도 카드마다 되풀이하지 않고 묶음 머리말에 한 번만 둔다. 이름순 정렬은
> 조경기사와 조경산업기사를 붙여 놓고, 두 묶음의 차례를 서로 맞춘다.
...
| 탭 | 내용 |
|----|------|
| `textbook` 쪽집게 노트 | **주제별 교재** (`gisa/essay_textbook.py` → `build_textbook`, 10분 캐시). 분류(topic_group) pill → 주제(topic_key) 아코디언. 대상: 2회 이상 출제 + 계산 + 재출제 유력(자연생태복원 193주제, 식물보호 51주제). 별 = 출제 회차 수(자연생태복원 5회↑ ★★★ / 3~4 ★★ / 2 ★ / 1 ☆), 회차 배지(`note_badges`), 정리 자료(자연생태복원 `freq58` 정의·이유·사례 58주제 · 식물보호 `topics` 개념·배경·시험포인트 51주제 · `calc` 공식·대입·함정)를 topic_key 로 이어 붙이고(글 안의 **주제키**가 우선, 없으면 대표 문항으로 추측), 정리가 없으면 최근 문항+모범답안. 걸러 보기(★★★/계산/재출제/⚠️), 기출 문항 펼침(`siblings?all=1`) |
...
| `wrong` 오답노트 | 카드: 큰 점수 배지(0점 빨강·부분 주황) · 문제 · **내 답안(색연필 첨삭) \| 모범답안** 나란히(820px↓ 위아래) · 총평 · 채점 결과/노트에서 빼기 버튼. 내 답안에 채점 결과와 같은 `penSheetHtml` 을 입힌다(`wrong_pen`) — 전에는 회색 글상자라 무엇이 틀렸는지 안 보였다. `_wrong_attempts()` — 문항마다 **가장 최근 응시**로 판정, 만점 미달만. **답을 안 쓴 문항은 넣지 않는다**(안 푼 것과 틀린 것은 다르다). 다시 풀어 만점이면 빠진다. "오답 재풀이"는 `source='오답'`(최대 20문항, 타이머 없음) |
| `history` 시험이력 | 세션 표 (모의/오답 배지) |
| `qna` 질의응답 | 실기 질문 (`cert_subject='실기'`) |
| `dvd` DVD | **식물보호 두 급수만** — 쪽집게 노트 바로 다음 탭. 안에서 분절 단추(`.dvd-sub`, `?dvd=pest\|pesticide`)로 둘을 고른다. 예전 주소 `?tab=pest`·`?tab=pesticide` 는 뷰가 그 칸을 연 DVD 탭으로 받는다. 칸 id(`tab-pest`·`tab-pesticide`)는 조각 JS 의 autoStart 가 찾으므로 바꾸지 말 것 |
| └ 해충 DVD (`pest`) | 사진으로 해충 이름을 고르는 4지선다(152종, 농작물 108 · 수목 44). 기본 칸. 보기 넷은 서버가 **같은 구분에서 먼저** 만든다. 사진을 누르면 확대(화면 맞춤 → 한 번 더 누르면 2배). 상세는 [docs/해충암기카드.md](docs/해충암기카드.md) |
| └ 농약 DVD (`pesticide`) | 농약명으로 살충제·살균제·제초제를 고르는 3지선다(92종). 상세는 [docs/농약암기카드.md](docs/농약암기카드.md) |

- **분류표·급수 묶음·별 기준·군집 임계값은 `gisa/essay_topics.py` 한 곳에 있다.**
  자격증마다 분류가 다르고(자연생태복원 8 / 식물보호 11), 기사와 산업기사는
  `SIBLING_GROUPS` 로 **한 덩어리로 묶어** 빈도를 센다(식물보호는 모수가 9회차가
  아니라 시험지 18장이다). 자세한 것은 [docs/식물보호기사.md](docs/식물보호기사.md)
  의 '실기 쪽집게 노트' 절. `tag_essay_frequency` 는 자연생태복원을 잠가 두었다
  (`LOCKED`, `--force` 필요) — 파라미터를 만지다 돌리면 726문항이 다시 잡힌다
- 쪽집게 노트 주제 제목은 **`gisa/essay_topic_titles.json` 이 최우선**이다(키는
  **`자격증|연도-회차-번호`** — 자격증을 빼면 충돌한다. 실제로 식물보호기사
  2023-1회 1번에 자연생태복원 제목이 달렸다). 발문을 규칙으로
...
- **'바로 채점'**(`essay_take` 머리글 토글, 기출·모의만): 문항 하나를 그 자리에서
  채점해 점수·모범답안·해설을 본다. **기본은 켜짐** — 실전처럼 끝까지 풀고 한 번에
  채점받으려면 끄면 되고, 끈 상태만 `localStorage.essayCheckMode` 에 남는다
  (`'0'`; 이 기능을 처음 낼 때 쓰던 `''` 도 꺼짐으로 받아 준다).
  학습(예상·오답)은 종전대로 늘 보인다
  - 기본값은 **서버가 `checked` 로 붙인다.** 화면이 그려진 뒤 JS 로 켜면 채점
    단추가 뒤늦게 나타나 깜빡인다 — 껐던 사람만 JS 가 감춘다
  - `essay_grade_one` 에 `session` 을 함께 보내 **점수를 세션에 적어 둔다.** 그래야
    제출할 때 다시 채점하지 않는다 — 한 문항씩 확인하며 20문항을 풀면 LLM 호출이
    두 배가 되고, 같은 답인데 화면에서 본 점수와 결과 화면의 점수가 달라진다
  - 건너뛰는 조건은 `graded_at` 만으로는 안 된다. **채점한 답안을
    `feedback['answer']` 에 남겨** 지금 답과 같을 때만 건너뛴다 — 채점해 보고 답을
    고쳤는데 옛 점수가 남으면 안 된다. `essay_grade_step`·`grade_session` 둘 다
    같은 규칙을 쓰고, 채점에 실패한 것은 `answer` 를 남기지 않아 다시 채점된다
  - `.eq-check` 는 `display:flex` 라 `[hidden]` 을 이긴다. `.eq-check[hidden]` 규칙을
    함께 두어야 실전에서 감춰진다(이 함정은 잡초 탭에서만 세 번 걸렸다)
- **퀴즈**(`?mode=quiz`, 기출고사 회차 카드의 세 번째 단추): 한 화면에 **한 문제씩**
  내고, 답을 쓰면 그 자리에서 채점해 점수·채점 포인트·모범답안·해설을 보여 준다.
  세션 `mode='quiz'`(시험이력에 '퀴즈' 배지), 타이머 없음. `essay_take` 가 같은 세션
  준비를 거친 뒤 `essay_quiz.html` 로 갈라 그린다
  - 채점은 `essay_grade_one` + `session` — '바로 채점'과 같은 길이라 점수가 세션에
    남는다. 응답의 `answer_html` 은 학습 화면과 같은 조각(`_essay_answer.html`)이다
  - 이어하기(`?resume=`)는 `_quiz_state()` 가 **채점한 답과 지금 답이 같은 문항만**
    결과를 되살린다. 채점 뒤 답을 고치면 '고친 답 다시 채점'으로 바뀐다
  - '결과 보기'는 풀기 화면의 제출과 같은 3단계(save → grade step → finish)다.
    채점한 문항은 건너뛰고 빈 답은 호출 없이 0점이라, 답만 쓰고 채점하지 않은
    문항만 LLM 을 부른다. 끝내야(`done`) 오답노트에 들어간다
  - 칩 색: 만점 초록 · 부분 주황 · 0점 빨강 · 답만 쓴 것 회색 테두리
  - **색연필 첨삭**: 채점하면 답안 칸이 '채점된 답안지'(`.qz-sheet`)로 바뀌어 빨간
    손글씨(Nanum Pen Script)로 틀린 구절에 줄을 긋고 ✗, 부족한 곳에 물결 밑줄,
    맞은 핵심에 밑줄 ✓, 옆에 짧은 첨삭, 끝에 "↳ 빠짐: …", 오른쪽 위에 점수(만점은
    동그라미)를 적는다. 총평·항목 판정도 손글씨로 그 아래 적는다. 답안지를 누르거나
    '답 고치기'로 textarea 에 돌아간다
    - 표시 자리는 채점 LLM 이 **답안에서 글자 그대로 옮긴 구절**(`marks[].quote`)이다.
...
### `[box]` 가 표로 끝나면 상자가 닫히지 않던 버그 (2026-09 수정)

`_render_box` 는 `'<div …>' + inner + '</div>'` 로 **줄바꿈 없이** 이어 붙인다.
상자 안이 마크다운 표로 끝나면 마지막 줄이 `| … |</div>` 가 되고, 뒤이어 도는
`_tables_anywhere` 가 그 줄을 표의 한 행으로 먹어 **닫는 태그가 사라진다.**

증상이 엉뚱한 데서 나타나 원인을 찾기 어렵다 — 상자가 안 닫히니 **뒤따르는
문항이 통째로 그 상자 안으로 말려들어가고**, 화면에서는 그 문항의 정답 보기
버튼·편집 폼·답 영역이 사라진 것처럼 보인다. 2026-1회 7번(조건 상자가 표로
끝난다)에서 겪었고, 12문항 가운데 편집 폼이 11개만 렌더되는 것으로 드러났다.

`_tables_anywhere` 가 표 블록의 마지막 줄에서 **뒤에 붙은 닫는 태그를 떼어
두었다가 표 뒤에 도로 붙인다.** 새 문항을 넣은 뒤에는 아래로 한 번 훑어
확인하면 된다.

```python
o = Counter(re.findall(r'<(\w+)(?=[\s>])', html))
c = Counter(re.findall(r'</(\w+)>', html))   # 열림 != 닫힘 이면 깨진 것
```

### 관리자 인라인 편집 (실기 학습 화면)

판독본의 오식이나 원도와 다른 그림은 **학습 화면에서야 드러난다**. 그때마다
고침 스크립트를 새로 쓰는 것은 느리고 무엇을 고쳤는지도 흩어지므로, 스태프는
그 자리에서 바로 고친다 — 눈 버튼 옆 연필(`btn-edit`) → 문제문·답 항목·답
서술·해설 네 칸.

- `essay_question_update`(`/gisa/<id>/essay/edit/<qid>/`, POST JSON, 스태프 전용).
  화면의 `{% if request.user.is_staff %}` 와 **서버의 is_staff 를 둘 다** 본다
- **답 항목은 칸을 하나씩 준다.** 한 칸에 줄 단위로 넣을 수 없다 — 계산형 항목은
  그 안에 줄바꿈(`계산)
…`)이 이미 들어 있어 줄로 가르면 항목이 쪼개진다.
  빈 칸은 저장할 때 버린다
- **고친 자리만 다시 그려 돌려준다**(`text_html`·`ans_html`). 새로 고치면 열어
  둔 답이 도로 접히고 읽던 자리를 잃는다. 답 영역은 화면과 **같은 조각**
  (`gisa/_essay_answer.html`)을 렌더해 두 벌이 어긋나지 않게 한다
- **답 항목을 고치면 `rubric` 을 비운다.** 채점 기준표는 답 항목에서 파생되므로
  (`build_rubric`) 예전 것이 남으면 화면과 채점이 어긋난다
- 답 서술이 400자를 넘으면 `<details>` 로 접어 둔다 — 자작 SVG 가 통째로 들어
  있어(뿌리분 그림 3,700자) 펼쳐 두면 다른 칸이 안 보인다
...
- **인쇄 시험지**의 박스 높이는 유형별로 다르다 (단답·빈칸 3줄 / 열거·표그림 6줄 /
  서술·계산 8줄, 줄 간격 32px). 종이는 나중에 덧붙일 수 없어 넉넉히 뽑아야 한다
- **화면 풀이(`essay_take`)는 유형과 상관없이 3줄로 연다**(2026-09). 괘선·줄 간격은
  시험지와 같고 줄 수만 다르다 — 화면은 쓰는 대로 `autoGrow` 가 늘려 주므로(6줄을
  쓰면 196px) 미리 벌려 둘 까닭이 없다. 서술형을 8줄로 열어 두었을 때는 빈 칸만
  화면을 채워 한 화면에 한 문항밖에 안 들어왔다. 두 화면이 CSS 를 공유하지 않으므로
  (`ln1`·`ln2`·`ln3` 은 `essay_take` 안에만 있었다) 한쪽만 고치면 된다
- `@media print`로 A4 최적화, `page-break-inside: avoid`로 문항 잘림 방지

#### 사진 곧게 펴기 (gisa/essay_rectify.py, 2026-09)

사진을 고르면 **판독 전에** 서버가 A4(10px/mm, 2100×2970)로 편 사진을 만들어
보여 주고(`essay_flatten`), 판독은 그 편 사진으로 한다(`GisaEssayUpload.flat_image`,
원본 `image` 는 그대로 둔다). 줄이 곧아야 판독이 낫고, 나중에 사진 위에 첨삭을
겹쳐 그릴 좌표도 이 편 사진 기준이다.

| 순서 | 방법 | `flat_method` |
|------|------|---------------|
| 1 | 인쇄 시험지 네 모서리의 **ArUco 마커**(DICT_4X4_50 의 0 왼위·1 오른위·2 오른아래·3 왼아래). 두 개만 보여도 편다 | `marker` |
| 2 | 마커가 없으면 **종이 가장자리**(밝고 채도 낮은 영역의 볼록껍질 네 극점). 옛 시험지·문서 스캔 앱 사진 | `edge` |
| 3 | 둘 다 실패하면 원본(EXIF 회전만) | `none` |

1·2 뒤에 **휜 종이를 한 번 더 맞춘다**(`_refine`). 답안 박스의 굵은 테두리와
**점선 괘선**이 곧아야 하므로, 휜 만큼을 가우스 가중 변위장으로 만들어 `remap` 한다.

- **마커 위치는 인쇄 CSS 와 짝이다.** `@page` 여백 10/10/12mm, 마커 9mm, `position:fixed`
  네 모서리. 본문이 덮지 않게 `<thead>` 12mm·`<tfoot>` 11mm 빈 칸을 둔다. 크롬
  PDF 로 뽑아 재 보니 8쪽 모두 계산 위치와 0.1mm 안에서 맞았다. 바꾸면
  `MARGIN_*_MM`·`MARKER_MM` 을 같이 바꿀 것
- 마커 무늬는 `MARKER_BITS` 에 박아 두어 **시험지 화면은 OpenCV 없이** 그린다
- 선 뽑기 기준 길이는 **3mm**. 6mm 로 두니 휜 구간에서 테두리가 끊겨 표본이 비었고,
  선 높이 한도 4mm 는 휜 테두리를 통째로 뺐다(지금 12mm)
- 실측: 실제 사진 8장(마커 없는 옛 시험지) 모두 `edge`, 테두리 휨 0.5~0.7mm → 0.1~0.2mm.
  인쇄 이미지를 휨 4mm·원근으로 비튼 합성 사진은 선 오차 중앙값 1.4mm → 0mm
  (마커 2개를 가려도 0.5mm). 장당 0.6~0.9초
- **사진 여러 장을 한꺼번에 고르면 요청이 겹쳐 같은 쪽 번호를 잡아 500 이 났다.**
  서버는 `_new_upload` 가 번호를 올려 다시 넣고, 화면은 한 장씩 차례로 보낸다
  (서버 작업자가 2개라 한꺼번에 보내면 다른 사람 화면도 멈춘다)
- 판독 전 업로드는 하루 한도에 들지 않는다(`transcribed=False`). 하루 지난 것은
  다음 업로드 때 치운다. 판독 전 사진은 ×로 빼면 파일째 지운다(`essay_upload_remove`)
- 서버 패키지 `opencv-python-headless==5.0.0.93`. **4.12 는 numpy 를 2.2 로 내리므로
  쓰지 않는다**(서버 numpy 2.4 를 kiwipiepy 가 쓴다)
- **가장자리로 편 것은 시험지인지 확인한다.** 곧게 선 가로 표본이 150개, 세로 30개에
  못 미치면 원본으로 판독한다(`info['rejected']`). 흰 책상·이불 위에서 종이와 배경이
  섞이면 엉뚱한 사각형으로 펴기 때문이다. 실제 시험지는 가로 583~1,417 · 세로 89~162,
  글만 있는 흰 종이는 가로 44였다. 마커로 편 것은 검사하지 않는다

#### 내 시험지 첨삭 (gisa/essay_overlay.py, 2026-09)

결과 화면에서 **입력한 답안 첨삭 ↔ 내 시험지 첨삭**을 오간다(시험지 사진으로 낸
세션만). 내 시험지 첨삭은 **시험지를 자르지 않고 쪽 그대로** 보이며, 첨삭 글을 그 쪽의
**빈자리에** 손글씨로 적는다. 쪽은 문항 차례로 정렬해 그 쪽 문항 카드들 바로 앞에
끼우고, 카드에서는 문제문·입력칸 답안지를 감춘다(항목 판정·모범답안·해설은 카드에 남는다).

> 거쳐 온 길: ① 쪽 전체 + 카드 감춤 → 총평·판정이 사라져 "부실하다" ② 문항 박스만
> 잘라 카드에 넣음 → "시험지를 자르면 현실성이 떨어진다" ③ 지금: 쪽 그대로, 빈자리 활용

**빈자리 찾기**
- 서버가 1mm 칸마다 잉크가 있는지(`ink_grid`, 210×297 packbits base64)를 판독 때 계산해
  `flat_info['grid']` 에 둔다(이전 쪽은 첨삭을 처음 열 때 계산해 저장). **점선 괘선은
  잉크로 치지 않는다** — 괘선 위에 글을 쓰는 것은 자연스럽다
- 화면이 손글씨 글꼴로 글 폭을 재어(`document.fonts.load` 뒤 canvas `measureText`)
  가리키는 곳에서 가장 가까운 빈 사각형(누적합)에 적고, 적은 자리를 채운다
- 거리 = 가로 + 2.2×세로 — 같은 줄 옆이 가장 자연스럽다. 총평은 가로 가중치 4(새 줄 선호)
- **문항마다 범위를 막는다**: 앞 문항 박스 아래 ~ **다음 문항 발문 첫 줄 전**(행 잉크
  12칸↑). 넓게 두었더니 총평이 다음 박스 안이나 발문 줄에 앉아 그 문항 것처럼 읽혔다.
  범위는 글 덩이의 **아랫변**에 건다(윗변에만 걸었더니 두 줄째가 발문 줄로 내려앉았다)
- 칸 좌표는 정수로 반올림할 것 — 76.3 같은 소수가 들어가 모든 자리가 '안 빔'이 됐다

**무엇을 어디에**
- 구절 표시(줄 긋기·✗·물결·✓) → 첨삭 글이 **같은 줄 바로 옆**에 들어가면 번호 없이 글만,
  아니면 번호 동그라미 + 가까운 빈자리에 "① 글". 표시·동그라미 자리는 먼저 채워 둔다
  (비워 두면 글이 제 동그라미 위에 앉았다)
- ✓ 에 "정답"·"맞음" 만 붙은 첨삭은 적지 않는다 — 쪽마다 수십 개 늘어 틀린 곳이 묻혔다
- 빠짐 → 마지막으로 쓴 줄 아래. **그 줄 오른쪽은 막는다**(총평이 이어 붙어 한 문장처럼 읽혔다)
- 문항 점수 → 인쇄 배점 '(5.0점)' 옆(발문 줄 ~ 박스 첫 줄), 만점은 동그라미
- 총평 → 박스 아래 틈. 못 적으면 **카드의 총평이 그대로 보이므로 목록에 되풀이하지 않는다**
- 전체 점수 → 첫 쪽 머리 왼쪽 위 빈자리(사용자 요청). 60%↑ 두 줄 밑줄
- 끝내 자리가 없는 첨삭·점수만 쪽 아래 목록(`.sv-more`)
- 실측(세션 146 재현 · 8쪽 20문항): 목록으로 넘어간 것 0, 총평 17/20 시험지에, 계산 0.3초
- 좌표는 편 사진 픽셀(2100×2970)

**자리는 LLM 에게 묻지 않고 이미지에서 찾는다.** 한 번 LLM 좌표로 해 보니 가로는
맞는데 세로가 반 줄씩 흔들렸다. 편 사진에서는 인쇄물이 곧아 분석이 확실하다.

1. 판독할 때(`transcribe_uploads`) 쪽마다 `page_layout` 을 계산해 `flat_info['layout']` 에 둔다
2. 답안 박스 = 굵은 실선 사각형 가운데 **안에 점선 괘선이 있는 것**(지문 [box] 는 괘선이 없다).
   답 ↔ 박스는 판독이 함께 준 `box_2d` 와 가장 많이 겹치는 것, 없으면 차례대로
3. 줄 = 괘선 사이 띠. **띠의 아래쪽 65% 만 본다** — 위쪽에는 윗줄 받침·꼬리가 내려와
   빈 띠가 '쓴 줄'로 잡혀 판독문 줄과의 짝이 한 칸씩 밀렸다. 잉크가 다른 줄의 20%
   아래인 띠도 빈 줄로 본다
4. 판독문 줄 ↔ 잉크 있는 띠를 차례로 짝짓는다. 줄이 더 많으면(줄 사이에 끼워 쓴
   덧말) **가장 짧은 줄을 윗줄에 붙인다** — 끝줄끼리 붙이면 뒤 줄이 모두 밀린다
5. 구절의 가로 자리 = 띄어쓰기로 나눈 낱말 ↔ 잉크의 큰 틈으로 나눈 덩어리. 개수가 안
   맞으면 잉크 누적량에 비례. 채점 구절이 판독문에 그대로 없으면(회원이 고친 뒤 채점)
   `_fuzzy_find` 로 60% 넘게 닮은 구간을 쓴다

- 실측(세션 146 · 8쪽): 박스 수 = 문항 수 8쪽 모두, 첨삭 45개 전부 자리 찾음, 쪽당 0.12초
- **PDF 저장은 인쇄 방식**(사용자 결정). 결과 머리글 'PDF 저장' → `erPrint()` 가 접힌
  모범답안·해설을 펼치고 문서 제목(=파일 이름)을 바꾼 뒤 `window.print()`. 서버에서 PDF 를
  만들지 않는다 — 시험지 위 첨삭 자리는 화면이 손글씨 폭을 재어 계산해 서버에서 재현이 어렵다
  - `@media print`: A4 10mm 여백, 인쇄 머리말(자격증·회차·응시일·이름·총점), 버튼·탭 감춤,
    점수 입력칸은 글자만, `print-color-adjust: exact`(괘선·첨삭 배경이 빠지지 않게)
  - 내 시험지 첨삭은 쪽마다 `break-before: page`, SVG 높이 272mm 로 A4 한 장에 한 쪽
  - '시험지 쪽만 PDF 저장'(안내 줄) = `body.print-sheets` — 카드·요약을 빼고 쪽만. 첫 쪽에
    나눔을 걸면 첫 장이 머리말만 남고 비었다 → 화면이 첫 쪽에 `sv-first` 를 달아 나눔을 뺀다
    (`:first-of-type` 은 div 가 섞여 듣지 않는다)
  - 확인은 Playwright(`channel='chrome'`) `page.pdf()` 로 했다 — 인쇄 창은 자동화로 못 연다.
    146 재현 기준 답안 첨삭 17장, 시험지 첨삭 22장, 시험지만 8장
- 휴대폰에서는 쪽이 340px 남짓이라 손글씨가 작다. 쪽을 누르면 900px 로 넓혀 옆으로 민다
- 사진에서 박스를 못 찾은 문항은 카드에 입력한 답안 첨삭이 그대로 보인다(`.qr.on-page` 만 감춘다)
- 카드는 `data-qno` 로 찾는다
- 이 기능 전에 올린 사진(편 사진·배치 없음)에는 탭이 나오지 않는다
...
## 작업형 CAD 탭 (`?tab=cad`, 2026-09)

모눈종이에 그리고 워드에 점 위치·작도 순서를 옮겨 적던 일을 화면에서 한다. 401 을
SVG 로 옮긴 방식(점 P1·P2 … 를 잡고 그 점들을 잇는다)을 손으로 하는 도구다.
`templates/gisa/_cad.html`(화면 전부) · `gisa/cad.py`(모눈종이 목록·저장 API) ·
모델 `CadDrawing`(회원별 도면) · `CadSymbol`(라이브러리).

- **CAD 가 아니라 제도판을 모사한다**(사용자 방침, 2026-09). 점·선·사각형·원·호 도구 단추는
  `hidden` 으로 감췄고 단축키(P·L·R·C·A)도 뺐다. **기호(라이브러리) 도구와 라이브러리 탭은 사용자 요청으로 되살렸다**(단축키 B).
  코드는 그대로라 되살리려면 `hidden` 만 떼면 된다. 아래 도구 설명은 그 코드에 대한 것이다
- **제도판은 도면(종이) 크기에 맞춘다**(사용자: "A1 으로 고정하지 말 것") — 종이 둘레에 여백(좌우 7U·위 6.5U·아래 = I자 폭 + 1.8U, 최소 3U)을 붙인 크기이고 — 아래 여백이 좁으면 I자 윗날이 종이 맨 아래까지 못 내려갔다(사용자: "끝까지 내릴 수 있게"), I자는 판의 좌우 레일에 걸리므로 판을 따라 맞춰진다. 한때 900×600mm 고정이었다가 되돌렸다. **I자는 판 위 틀 밑 ~ 판 아래 끝까지 오르내린다**(`setBarY`). **아래 펜 받침은 뺐다** — 보기는 좋지만 그리는 자리를 먹었다(사용자). '맞춤'은 판 전체를 잡되 **위에 붙인다** — 화면이 세로로 남으면 남는 자리를 아래로 몬다(가운데 맞춤은 위 여백이 넓어 보였다, 사용자). 위 여백은 1.2U, 좌우·아래 3.4U. 스케일자·템플릿은 실물 mm 를 축척으로 바꾼 크기, 삼각자는 종이 높이 비율이다
- **제도판**(`#cadBoard`, `boardSvg`·`boardGeom`): 사용자가 보여 준 탁상 제도판을 평면으로 종이 둘레에 그린다 — 흰 판(그림자),
  좌우 알루미늄 레일(왼쪽 레일에 모눈 눈금), 위쪽 얇은 틀, 아래 펜 받침 홈, 네 모서리 검은 마개. 종이 네 귀의 마스킹 테이프는 종이
  위라 `gridSvg` 가 화면용(`id==='cg'`)에만 얹는다(SVG 내보내기에는 없다). **I자 양 끝 블록이 좌우 레일에 걸리도록** I자 길이를
  제도판 폭에 맞췄고, '맞춤'도 제도판 전체가 들어오게 잡는다. I자는 **처음 모습(알루미늄 몸통·짙은 끝 블록)으로 되돌리고
  끝 블록에 실사 조임 다이얼만** 얹는다(`DIAL_IMG`, GPT 이미지 96px base64 — 사진처럼 은색 몸통·레버·갈고리로 바꿨다가 '이전 형태로, 레버는
  빼고 다이얼만 실사로' 요청). 왼쪽 레일 안쪽에 짙은 눈금 띠, 레일 아래쪽 검은 멈추개. **아래 끝의 펜 받침도 뺐다**(공간 절약, 사용자). 판은 `pointer-events:none` 이라 누름을 가로채지 않는다
- **시설물 수량표 기호**(라이브러리 탭 둘째 묶음, `FAC` 67종): 조경기사 작업형 도면 자료(`GisaDrawingRef`, 주로 '성운' 답안)의
  '시설물 수량표' SVG 에서 기호·시설명·규격을 뽑았다(`tools/cad_fac_extract.py` → `fac_rows.json`, `tools/cad_fac_build.py` 가 Playwright 로
  기호의 bbox 를 재어 `fac_lib.json`). 이름이 같으면 한 번만, 규격에 '가로×세로'가 있는 것을 우선. **m 로 세는 선 시설(맹암거·배관·배선·펜스)은 뺐다.**
  크기: 가로×세로(mm)면 그 상자에 맞춰 가로·세로를 따로 늘리고, Ø·D 는 지름, L 은 길이, 높이만 적힌 것은 평면 크기를 어림(등·안내판 0.6~1.5m,
  놀이시설 3m, 그 밖 1.5m). 항목은 `{t:'svg', svg, bb, w, h}` — `itemSvg` 가 원래 px 그림을 w×h(m)로 늘리고 기준점을 가운데로 둔다.
  선 굵기는 `vector-effect="non-scaling-stroke"` + 1.1px 로 고정(가로·세로를 따로 늘려도 굵기가 흔들리지 않게). 자료가 늘면 두 스크립트를 다시 돌려 `FAC` 를 바꾼다
- **A2 트레이싱지**(종이 고르기의 둘째 묶음, `cad.TRACES`): 594×420mm 가로, 눈금 없는 반투명 종이(`traceSvg` — 제도판이 비친다, 네 귀 테이프). 축척 8가지(1:100~1:800) 가운데 고르면 종이 크기가 0.594·S × 0.42·S m 가 된다. 종이 1cm 를 눈금 한 칸(`step` = S/100 m)으로 잡아 스냅은 모눈종이와 같은 결이다(보이지 않는 눈금에 붙는다). 키는 `a2-<축척>` 이고 서버 `SHEET_KEYS` 에도 들어 있다
- **모눈종이 PDF 17장은 모두 빈 눈금 종이다** — 다른 것은 좌표 범위와 축척뿐이라
  PDF 를 싣지 않고 `cad.SHEETS` 의 범위로 화면이 눈금을 그린다. 06 주택정원 PDF 는
  제목이 '(436)' 이지만 파일 이름·출제 표를 따라 434 로 두었다
- 도면 = 점 목록 + 작도 순서(ops) JSON. **선·원·호가 점 번호를 가리키므로** 점을
  옮기면(선택 도구로 끌기, 점 목록에서 고치기) 걸린 선이 모두 따라온다
- 도구: 점 · 선(이어 긋기) · 사각형 · 원(반지름/지름, 중심 뒤 한 번 더 누르면 크기) ·
  호(방향·반지름, 빈칸=반원) · 기호(기준점·회전) · 글자. 선 종류 5(실선·일점쇄선·
  이점쇄선·보조선·점선) × 굵기 3. **쇄선은 끝이 반드시 긴 마디**(`fitDash`, 사용자 — docs/조경기사.md 의 401 부지 경계 규칙과 같다):
  선 길이에 맞춰 주기 수 n 을 골라 무늬를 늘이거나 줄이고(열린 선 = n 주기 + 긴 마디, 원 = n 주기), 사각형은 변마다 따로 긋는다.
  짧은 선도 점 하나는 들어가게 줄이고, (주기+마디)의 35% 보다 짧으면 실선. 무늬는 긴 마디 12U · 틈 1.6U · 점 0.3U —
  "점이 너무 많다, 선이 더 길고 점은 몇 개 안 되게"(사용자). 401 원본(마디 12~15m · 틈+점+틈 3.7m)과 같은 비 휠 확대, 끌어 이동, 스냅(F9) 켜면 눈금 교차점만,
  끄면 0.1m 단위(1:20 종이는 0.01m). 스냅 간격 목록에는 **0.1m·0.2m 를 늘 둔다**
- **I자(평행자)**: 실물처럼 그렸다 — 알루미늄 몸통(불투명, 밑의 도면을 가린다)·양 끝 고정 블록,
  긋는 쪽에만 투명 아크릴 날. 몸통을 끌거나 ↑↓(Shift 는 5칸)로 위아래로만 옮기고, **몸통을 눌러도
  점은 안 찍힌다**. **투명 날을 따라 끌면 연필로 긋듯 수평선**(선 도구의 선 종류·굵기)이 생기고,
  날을 끌지 않고 누르면 평소 누름이다. 날 윗변(barY) 16px 안을 누른 점은 그 높이에 붙는다(`snapXY`).
  자리와 켜짐은 도면이 아니라 화면 상태라 localStorage(`cadBar`)에 둔다
- **샤프 0.9 / 0.5**(도구 `pen9`·`pen5`, 단축키 9·5): 고르면 I자를 펴고, **I자 위에서 오른쪽 버튼을
  누른 채 끌면** 날을 따라 선이 그어진다(실제 제도처럼 자에 대고 긋는 연습). 굵기는 선 굵기 표(`WD`)의
  `p9`(0.3)·`p5`(0.17)라 작도 순서에 '실선(샤프 0.9)' 로 적힌다. 선 종류는 샤프 칸에서 고른다.
  **샤프를 든 동안에는 끌어도 화면이 움직이지 않는다** — 날이 화면에서 9px 남짓이라 조금만 빗나가도
  화면이 밀렸다. 날 위아래 40px 안에서 끌면 왼쪽·오른쪽 어느 버튼이든 긋고, 몸통을 왼쪽으로 끌면 자를
  옮기며, 그 밖은 아무 일도 안 하고 안내만 띄운다. 화면 이동은 휠 버튼·Space 끌기
- **선을 그을 때 기존 요소에 붙는다**(`edgeCross`): 샤프로 자의 변을 따라 그을 때 시작·끝이 기존 선·사각형 변과 자의 변이 만나는 자리에서 화면 10px 안이면 그 교차점에 붙는다(붙으면 미리보기에 빨간 동그라미). 기존 점에 붙는 것(`hitPoint`)이 먼저다. **원·호 둘레도 같다**(`allCircles`·`lineCircleTs`) — 그을 때 둘레에 붙고, 점 메뉴의 연장·줄이기 후보에도 원·호가 나온다(중심에서 뻗은 선이 원을 지나치거나 모자라던 것, 사용자). 호는 호 위의 교점만 친다
- **점 합치기**(`mergeCands`·`mergePts`·`ctxMenu`): 샤프로 점을 고른 뒤 **그 점에서 오른쪽 버튼** → 그 점에 이어진 선의 연장선(또는 선 위, 화면 6px 안)에 있는 점을 가까운 차례로 메뉴에 보이고, 고르면 고른 점을 그 점으로 합친다 — 그 점을 쓰던 것(`REF_KEYS`)이 모두 새 점을 가리키고 원래 점은 지운다. 길이 0 이 된 선·사각형·호는 버린다. **합친 뒤 번호를 다시 매긴다** — 차례는 그대로 두고 빈 번호만 메운다(P2 를 합치면 P3→P2 …, `remapPts`). 끝이 어긋나게 그린 선을 잇는 용도(사용자). 같은 메뉴 아래에 **교차선까지 연장**(`extCands`·`extendTo`): 그 점에 이어진 선을 (반대 끝 → 고른 점) 방향으로 늘려 만나는 선·사각형 변을 가까운 차례로(최대 6) 보이고, 고르면 교차점에 새 점을 두고 **이 선의 끝만** 옮긴다(고른 점을 다른 것도 쓰면 틀어지지 않게; 아무도 안 쓰게 된 점은 지운다). 뒤에 `compactPts` 로 번호를 메운다. 그 아래 **지난 교차선까지 줄이기**(`extCands(n, true)`): 이 선이 이미 지나온 교차선(선·사각형 변)까지 끝을 되돌린다(삐져나온 끝 자르기). **겹친 점**: 누른 자리 화면 8px 안에 점이 여럿이면(몇 mm 차이로 찍혀 겹친 것) 그 점들을 모두 함께 보고 후보를 모은다(`P101: …` 꼴) — 선과 이어지지 않은 점이 골라져 후보가 비던 문제(사용자). 맨 위에 **겹친 점 합치기**(선이 가장 많이 걸린 점으로 모음, `mergePts` 는 여러 점을 한 번에 받는다). 맨 아래 **삭제**(`delPoint`): 그 점과 그 점을 쓰는 선·원·호·글자를 함께 지우고 번호를 메운다
- **샤프로 끌지 않고 왼쪽으로 누르면 점을 찍는다**(`penDot`) — 자의 변 곁(40px)이면 변 위에, 아니면 스냅 자리에.
  이미 있는 점이면 고르기만 하고, 자가 덮은 곳(I자 몸통·삼각자 안)에는 찍지 않는다
- **I자 날은 위아래 둘이다**(실물처럼). 폭(`barW`)은 눈금의 배수라 윗날이 눈금 위면 아랫날도 눈금 위다
  (401 은 8m — 4m 는 좁다는 지적으로 넓혔다). 긋기·붙기는 가까운 날로 간다. 표시 'Y 30 · 38'
- **몸통·삼각자 위 커서는 실사 손 사진**(48px PNG base64, 편 손 → 끄는 동안 오므린 주먹). GPT 이미지 모델
  (`background='transparent'`)로 손등 사진을 받아 손목을 잘라 내고 48px 로 줄인 뒤 옅은 그림자를 붙였다.
  만화 같다는 지적에 SVG 손을 버렸다. 이전의 SVG 커서 때 배운 것: CSS `url("data:image/svg+xml,…")`
  **안의 SVG 따옴표는 홑따옴표여야 한다** — 겹따옴표를 퍼센트 인코딩 없이 두면 url("…") 이 거기서 끊겨
  선언 전체가 버려지고, 아무 오류 없이 기본 커서가 나온다(실제로 당했다)
- **자가 덮은 곳은 지워지지 않는다**(`covered` = I자 몸통·두 날 + 삼각자 판 — 가운데 삼각형 구멍은 종이라 지워진다).
  지우개는 문지른 원판 안에서도 덮인 구간을 빼고 지운다(`freeRuns` 로 0.03U 간격 표본). 자가 지우개판 노릇을 한다.
  자에 대고 그은 선(변 위)은 덮인 것으로 치지 않는다. 지우개로 자를 누르면 옮긴다(삼각자는 몸통 어디든).
  **지우개는 서 있다가 누르는 동안 기울어진다**(커서 두 벌 stand/press, 루트에 `.erasing`), 빨간 원도 누르는 동안만 보인다.
  자 위에서는 지우개 원을 감춘다(`onRuler`) — 커서는 손이 되고 끌면 자가 옮겨지는 곳이다.
  한때 '삼각자 밑도 지워지게' 로 잘못 알아들어 반대로 만들었다 — 사용자의 뜻은 '지워지면 안 된다' 였다
- **모따기 명암**: 구멍 둘레(구멍 쪽으로 파여 내려감)와 바깥 둘레(바깥쪽으로 깎임)에 0.5U 띠를 두고, 깎인 면이 왼쪽 위 빛을
  향하면 밝게·등지면 어둡게 칠한다. 띠는 내심 기준 닮은꼴로 넓히고 줄여 모서리가 맞물린다. (흰 반사광 선은 그린 선 위에서
  회색 줄로 보여 뺐다. 그림자·푸른 띠·눈금은 '이전이 좋다'로 되돌렸다)
- **뒤집기는 그 자리에서**: 차지한 폭은 그대로, 직각 모서리를 예전 밑변 끝으로 옮긴다(눈금에 붙임)
- **삼각자 회전 아이콘은 뺐다**(사용자 요청 — 그 자리에 삼각형 구멍). `rotateTri` 는 남아 있지만 부르는 곳이 없다
- **지우개 둘**(`erase5` 미세 지우개 연필 0.6U · `erase` 일반 2.2U, 단축키 W·E): 누른 채 문지르면
  원판에 닿은 부분만 지운다 — 선은 잘리고(`cutLine`), 원·호는 같은 원 위의 호로 남는다(`cutArc`,
  SVG 호 끝점 표기 → 중심 변환은 `arcGeom`). 사각형은 네 선으로 풀고, 기호·글자는 통째로, 지운 자리의
  빈 점도 지운다. 크기는 종이에 대한 실제 크기라 확대하면 세밀해진다. 한 번 문지른 것이 되돌리기 한 칸이고,
  아무것도 안 지웠으면 그 칸을 도로 뺀다
- **샤프·지우개를 들면 커서가 그 도구의 실사 사진이다**(`.cad-tool-pen9 #cadSvg` 등, PNG base64 64·56px).
  샤프·지우개 연필은 **몸통 가운데를 잘라 내고 촉과 윗부분(클립·캡)을 이어 붙여** 짧고 굵게 했다 — 윗부분만 잘랐더니
  끝 모양이 없어졌다는 지적을 받았다. 45° 로 누워 있어 대각선을 따라 잘라 그만큼 당기면 이음매가 안 보인다.
  찍히는 점(hotspot)은 샤프심 끝·지우개가 닿는 끝 — 그림에서 가장 왼쪽 아래 화소를 찾아 잡았다. 샤프는
  사용자가 보여 준 제도 샤프를 따랐다: **0.9 = 머스터드 노랑(몸통·홈 그립 모두), 0.5 = 무광 검정**, 크롬 클립·캡·촉.
  **샤프는 자의 반대쪽으로 눕는다**(`penLean`) — 가까운 자 변의 어느 쪽이 자인지 `covered` 로 보고, 한 벌의 커서를
  좌우(`.lean-h`)·위아래(`.lean-v`)로 뒤집은 네 벌 가운데 고른다(I자 윗날 = 오른쪽 위, 아랫날 = 아래로).
  사진은 GPT 이미지(`background='transparent'`), 로고·글자는 넣지 않게 했다. I자 날 위에서도 도구 커서가
  보이도록 날의 커서 규칙은 두지 않는다(몸통·삼각자 위만 손)
- **실물 도구 아이콘**(샤프·지우개·삼각자·I자)은 그라데이션·금속 촉·슬리브를 넣은 입체 SVG(`svg.real`, 24px)다. **삼각자 위의 각도 글자(45°·60°)는 뺐다**(사용자 — 필요 없음).
  20px 에서는 사진을 줄이면 뭉개져 벡터로 그렸다. 일반 아이콘 규칙(`fill:none; stroke:currentColor`)이 표현 속성을
  덮으므로 그 규칙은 `svg:not(.real)` 에만 건다
- **삼각자 45° · 60°**(도구 줄 단추, 켜고 끄는 것 — 도구가 아니다): I자 윗날에 얹혀 좌우로만 민다(I자를
  옮기면 따라온다). 손으로 끌어 밀고, '뒤집기'(빗변 좌우), 60° 자는 '빗변 30°'(눕히기)로 바꾼다. **무색 투명**
  아크릴(처음 주황 → '투명으로' → 다시 '약간 노란 톤으로', 채움 rgba(255,196,60,.22)·테두리 #c9a24a). **가운데 삼각형 구멍**은 둥근 구멍을 '막을 것'이라 했다가, 뒤에 '회전 아이콘 대신 세 변에서
  균형 있는 삼각형 구멍'으로 다시 뚫었다 — 내심을 중심으로 세 변에서 같은 거리(내접원 반지름의 70% — 처음 45% 였다가 '많이 줄여' 요청)만큼 들인 닮은꼴(`G.hole`),
  구멍의 세 변도 `guideEdges` 에 들어가 대고 긋는다. **삼각자 위에서는 점·선·원 등 어느 도구도
  작용하지 않고 옮기기만 된다**(누르기만 해도 통과하지 않는다, 도구 미리보기도 없다) — 사용자 요청
  자리는 localStorage(`cadTri`)
- **삼각자 크기는 종이 높이에 대한 비율**(사용자: "스케일자·템플릿은 그대로, 삼각자는 종이 크기에 맞게") — 직각을 낀 긴 변 = 종이 높이 × 설정 %(45° 30~200% 기본 50% · 60° 40~200% 기본 70%, `SET.triPct45`·`triPct60`). 설정 칸 옆에 도면 m 와 종이 위 cm 를 함께 보인다. **45° 자는 직각변을 굵은 눈금(5m) 배수로 맞춘다** — 꼭짓점이 눈금 교차점에 오게. 한때 실물 cm(45° 12cm · 60° 45cm)를 축척으로 바꿨는데 종이가 작으면 자가 종이를 넘었다
- **삼각자 수직변의 X 를 늘 보인다**(`.tri-x`, 직각 모서리 곁 바깥쪽 — 꼭대기에 두었더니 종이 밖으로 나가 안 보였다).
  끄는 동안에는 붉은 바탕으로 크게, 수직변 연장 점선과 함께 실시간으로 바뀌고 상태줄에도 뜬다
- **자 위치 숫자(I자 몸통 'Y a · b'·윗날 Y 표시·삼각자 X 표시)는 설정 '자 위치 숫자 보이기'로만 보인다 — 기본은 감춤**(사용자 요청,
  `.cad:not(.ruler-nums)`). 끄는 동안의 값은 상태줄에 늘 뜬다. 아래 두 줄은 켰을 때의 모습이다
- **I자 윗날(상단 수평자)의 Y 도 늘 보인다**(`.bar-y`, 왼쪽 끝 블록 곁 날 위). 끄는 동안 붉은 바탕으로 크게, 상태줄에도 뜬다
- **원형 템플릿**(도구 줄 '템플릿', `#cadTpl`, localStorage `cadTpl`): 사용자가 보여 준 서클 템플릿(폭 235×111mm, 구멍 39개
  1~12mm 0.5 간격 · 13~22 · 23·25·28·30·33·36)을 사진에서 재어(22cm = 614px) `TPL_HOLES` 에 적었다. 종이 위 mm 를 축척으로 m 로 바꾸므로
  1:430 에서는 종이를 거의 덮는다(실물 비율). 구멍은 표기 + 0.5mm(연필 여유), 그려지는 원은 표기 지름.
  **샤프로 구멍 안을 누른 채 돌리면 돈 각도만큼 호, 한 바퀴(97%↑)면 원**이 된다(`tplTrace` — 각도를 모은다). 원은 중심 점 하나,
  호는 양 끝 점을 만든다. 판을 끌어 옮기면 **잡은 곳에서 가장 가까운 구멍의 중심이 눈금에 붙고** 그 좌표를 실시간으로 보인다.
  판은 자처럼 가린다(`covered`·`onRuler` — 지우개·점 찍기가 안 된다, 구멍은 종이). 구멍은 evenodd 로 뚫어 누르면 종이다
- **도형 템플릿**(도구 줄의 그림 단추 둘 — 원형(`#cadTplBtn`)·도형(`#cadTplGeo`, 판에 △□⬡). 콤보는 없앴고 감춘 `#cadTplKind` 가 종류를 든다; 같은 단추를 다시 누르면 치우고 다른 단추는 그 자리에서 바꾼다, `TPL_KINDS.geo`): 원 15 · 정삼각형 14 · 정사각형 15 · 정육각형 15.
  사용자가 준 사진이 230px 라 **구멍 번호를 읽지 못해** 구멍 크기를 재고 흔한 계열에 맞췄다(원 1~10·12~20, △ 2~10·12~20,
  □ 2~10·12~22, ⬡ 1~10·12·14·15·16·18) — 선명한 사진이 오면 고칠 것. 표기는 원 = 지름, □·△ = 한 변, ⬡ = 마주 보는 변 사이.
  다각형 구멍을 돌리면 한 바퀴면 닫힌 도형(꼭짓점마다 점 + 선), 덜 돌리면 지나간 변만 선으로 긋는다(`polyTrace` — 중심에서 뻗은
  반직선과 둘레의 교점 `polyAt`). 계산기에도 모양 고르기가 있어 그 모양 구멍들로 셈한다
- **템플릿 회전 핸들**(`.tpl-rot`, 윗변 가운데 바깥 9mm 에 점선 줄기 + 손잡이): 일반 그림 앱처럼 **끌면 판 가운데를 축으로 자유 각도로**
  돈다(`tplrotd`, 1° 단위 정수, Shift 는 15° 씩). 끄는 동안 각도를 붉게 보인다. 처음에는 판 안 아이콘을 눌러 90° 씩 돌렸다(`rotateTpl` 은 남았지만 안 쓴다).
  판 위 좌표는 모두 `tplMap(u, v)`(mm → m, 회전 포함)·`tplLocal(p)`(역변환)을 거친다 — 구멍 중심·다각형 꼭짓점(`rotAbout`)·
  판 테두리·자 눈금·인쇄 글자(판과 함께 돈다). 구멍 번호 글자와 중심 맞춤 표시는 도면 축에 맞춰 둔다. `tpl.x·y` 는 돌리기 전
  왼쪽 위라 끌어 옮기기(`startTplDrag`)는 손대지 않았다
- **구멍 안에서는 샤프 커서를 지킨다**(`.cad.in-hole #cadSvg * { cursor: inherit !important }`) — 구멍은 뚫려 있어 밑의 삼각자·I자
  몸통이 손 커서를 띄웠다. 구멍 안에서 샤프는 **구멍 가운데 쪽으로** 눕는다(판이 바깥에 있다) — 돌아가며 방향이 바뀐다
- **접선 맞춤·끝 맞춤**(원 구멍만): 템플릿을 옮길 때 잡은 구멍의 원이 도면의 선·사각형 변(보이는 것 — 보조선도, 보조선 레이어가 켜져 있을 때)이나 자의 변에 12px 안으로 다가가면
  중심을 옮겨 **정확히 접하게** 한다(접점이 그 선분·자의 변 **범위 안**일 때만 — 무한 직선으로 보았더니 멀리 있는 빗변 연장선에 붙어 눈금에서 벗어났다; `tangentCenter` — 선 방향으로는 눈금 자리를 지키고, 기울기가 다른 두 선에 함께 닿으면 두 평행선의
  교점 = 모서리 라운드). 중심만 눈금에 붙이면 반지름(번호÷2×축척)의 끝수만큼 원 바닥이 선에서 떠 선과 호가 층졌다(사용자 지적).
  호는 시작·끝을 원의 위·아래·좌·우, 접한 선의 접점, 원 위의 점에 14px 안(최대 6°)이면 붙인다 — 각도 상한이 없으면 작은 구멍에서 14px 가 37° 라 135°쯤이 180°로 건너뛰었다(`arcCands`·`snapAng`·`traceSweep`)
- **스케일자**(도구 줄 '스케일자', `#cadSr`, localStorage `cadSr` = {x, y, rot, roll}): **사용자 실물(사진·단면·실측)을 그대로 옮긴 삼각 스케일**(2026-09).
  **단면은 세 갈래 별 모양**이다 — 세 면이 가운데 둥근 홈에서 안으로 꺾여, 종이에 닿는 꼭짓점 쪽 부분은 완만하고(약 45°) 능선 쪽은 가파르다(약 67°).
  **종이에 닿는 밑면 폭(꼭짓점 사이)은 14.5mm**(사용자 실측). **능선은 가운데가 아니라 2/3 자리**(`SR_HA`) — 눈이 자 앞쪽 위에 있어 측정 기준인 윗면(A)이 폭의 2/3, 아랫면(B)이 1/3 로 보인다(사용자). 윗면은 밝게, 아랫면은 어둡게(`srFaceA`·`srFaceB`), 능선 너머에 그늘 선. 아랫면의 종이 쪽 눈금 글자는 너무 작아지지 않게 눌림 하한 0.36. 위에서 본 반쪽 7.25mm = 종이 쪽 눈금 4.6 · 홈 0.8 · 능선 쪽 눈금 1.85mm(`SR_SEG`,
  단면 사진에서 잼) — 그래서 1/100·1/400 은 넓게 잘 보이고 1/200·1/300 은 눌려 덜 보인다. 눈금·글자는 면 좌표(`SR_F` 25mm)로 그리고
  구간마다 줄여 화면에 놓는다(글자도 그 비율로 세로를 누른다). **두 면이 만나는 모서리는 칼날이 아니라 눈금 없는 흰 평면(랜드)**이고 연필은 종이 쪽 랜드에 대고 긋는다 — 위에서도 흰 띠로 보인다(`SR_LAND` 0.45 · 능선 `SR_RIDGE` 0.4mm, 사용자 사진). **단 종이에 닿는 두 모서리(위·아래)에는 흰 띠도 회색 테두리선도 두지 않는다** — 눈금이 모서리에서 바로 시작한다(사용자; `seg` 의 `LD`=0). 테두리선은 둥근 양 끝에만 긋는다. 대신 화면 위쪽 모서리 바깥(종이 쪽)으로 0.9mm 옅은 그늘(`srTopSh`)을 번지게 해 흰 종이와 경계를 드러낸다. 숫자 **자리는 면 좌표 그대로(입체감)**, 종이 쪽 숫자 **글자만 세로 0.72 로 덜 누르고** 크기를 그만큼 줄인다 — 면 비율(≈0.42)로 누르면 획이 가로로 얇아져 "인쇄물처럼 흐리다"는 지적을 받았다. 덧씌운 테두리도 번져 뺐다. 굵기 600(종이 쪽)·500(능선), 검정. 세 면 가운데 둥근 홈을 파 면마다 색칠 — 면 하나에 축척 둘: **빨강 1/100·1/200 · 청록 1/300·1/400 ·
  검정 1/500·1/600**. 꼭짓점마다 이웃 두 면의 축척이 만난다(1/200|1/300 · 1/400|1/500 · 1/600|1/100).
  **제도판에서는 한 면을 바닥에 눕히므로 위에서 보이는 것은 두 빗면과 가운데 능선이다**(사용자 지적 — 처음에 한 면을 평평하게 그렸다가 고쳤다).
  **종이에 닿는 두 모서리만** 치수를 잰다(`srEdges`; 능선은 떠 있어 못 쓴다). **스케일자로는 선을 긋지 않는다**(사용자) —
  `srEdges` 는 `sr: true` 로 샤프 긋기 후보·점 붙기(`snapXY`)에서 빠지고, 템플릿 접선 맞춤에만 쓰인다. 모서리를 누르면 `srmark` 드래그가 되어 끌어도 선이 없다. 마킹으로 받는 모서리 곁은 `srMarkEdge` — 화면 8px 이면서 **자 두께의 22% 이하**(12px 고정이었을 때 자가 작게 보이면 몸통 전부가 마킹 범위라 샤프로 끌어도 안 옮겨졌다). 몸통 위에서는 십자 커서를 감춘다. **축척을 미리 고르지 않는다** — **앞쪽(화면 왼쪽) 여백, 크게 보이는 면의 색 띠 높이에 있는 축척 전환 단추**(`.sr-roll`, Shift 는 반대)를 누를 때마다(단추 밑에 **화면 반지름 13px 이상의 투명 누르는 자리** — 맞춤 배율에서는 단추가 작아 손 커서가 잘 안 떴다; 배율이 바뀌면 `zoomAt`·`fit` 이 다시 그린다) **크게 보이는 윗면(2/3)의 눈금이 1/100 → 1/200 → … → 1/600** 으로 바뀐다(`SR_CYCLE`: [바닥 면 `sr.roll`, 앞뒤 돌림 `sr.swap`] 여섯 상태). 굴리기만으로는 1/200·1/400·1/600 이 늘 좁은 아랫면에 있어 안 보였다(사용자). `sr.swap` 이면 판 전체를 180° 돌려 그리고(`srRot`) 능선을 1/3 자리로 옮겨(`srHA`) 크게 보이는 쪽이 화면 위에 오게 하며, 밝은·어두운 면·그늘·손잡이 자리도 함께 바꾼다. 도구 줄의 종류 고르기(`#cadSrKind`)에서 **1:N 사용자 지정**(예 1:430, `sr.custom`)도 고를 수 있다 — 네 눈금이 모두 1:N, 파란 홈, 굴림 손잡이 없음. 넣은 N 은 목록에 남는다(localStorage `cadSrCustom`). 눈금 간격은 `srGrad`(표에 없으면 종이 위 1mm·12mm 안팎). 사용자 지정 자의 눈금 길이는 `srRun` — 300mm 가 129m 처럼 끝수가 남으면 반대쪽 눈금이 비껴 서므로 숫자 간격의 배수(1:430 → 120m)로 끊는다.
  판 317mm, 눈금 300mm, **양 끝 여백 8.5mm**(실측은 12·13mm 였지만 "길다, 최대한 짧게" — 축척 표기가 겨우 들어가는 만큼). 표기 '1/300' 은 여백 가운데에서 0.8mm 바깥쪽, **작은 숫자 줄과 같은 높이**(`lb` = `sm`), 'm' 은 **표기 마지막 '0' 바로 위**에 작게 — 표기 폭(`lbHalf`, 숫자 0.556em · '/' 0.278em)을 셈해 자리를 잡는다. **양 끝 네 모서리는 둥글게**(반지름 2mm, `RR` 윤곽 + `srClip`) — 한 면의 두 축척은 서로 180° 돌린 모양이라 **한쪽 끝 눈금과 다른 쪽 0 이 같은 자리**다.
  숫자 위 **작은 숫자는 반대쪽에서 읽는 값**이다. **눈금·숫자의 꼴은 실물 사진(1/100 면 정면)을 잰 비율 그대로**(사용자: "형태를 참조하라") — 면 폭 10.8mm 기준 작은 눈금 1.2 · 0.5 눈금 2.5 · 1m 사이 긴 중간 3.2 · 숫자 눈금 3.9 · 작은 숫자 가운데 3.7(높이 1.4) · 큰 숫자 가운데 6.1(높이 2.2). **→ 이 값은 눈대중이라 틀렸다. 사진을 픽셀로 다시 재니(면 10.8mm = 60px) 작은 눈금 3.0 · 0.5 눈금 4.5 · 숫자 눈금 5.6 · 작은 숫자 3.6~5.3 · 큰 숫자 5.9~8.4 — 눈금이 길게 내려오고 숫자가 눈금 끝에 바짝 붙는다(사용자: "사진을 잘 보라"). 눈금 비율은 반드시 사진 화소를 세어 정할 것.** 숫자 눈금은 **작은 숫자 두 자리 사이**를 지나며(2│9) 글자를 눈금 양옆으로 갈라 닿지 않게 한다(한 자리면 눈금 오른쪽, **세 자리면 가운데 숫자가 눈금 한가운데** — 1[4]0). 숫자·표기는 모두 굵게 하지 않는다(사용자). 글자는 세로 0.72 로 덜 누르고 크기를 줄여 사진의 높이 비율에 맞춘다(`fsz`). 눈금 표 `SRG` = [작은, 중간, 긴 중간, 숫자] —
  1/100 0.1·0.5·1, 1/200 0.1·0.5·1·5, **1/300·1/500 0.2**·1·5·10, 1/600 0.5·1·5·10(사진을 세어 정함), 1/400 0.5(사진이 흐려 미확정). **자 오른쪽 끝 10mm 는 0.1m 촘촘한 눈금**(`SR_FINE`, 1/300~1/600 — 사진에서 회색 띠로 보인다). 1m(실제) = 종이 1000/S mm = 도면 denom/S m.
  몸통을 끌면 0 눈금이 눈금에 붙고, 윗변 바깥 가운데 회전 손잡이(하나만 — 사용자)를 **끌면 자유 회전**(정수°, Shift 15°), **누르기만 하면 90° ↔ 0°**(사용자; 180°↔0° · 다음 45° 를 거쳐 바뀜). 몸통은 자처럼 가린다(`covered`·`onRuler`)
  **재는 면(크게 보이는 면)이 늘 빛을 받아 밝다**(`darkA = SW`, 사용자) — 한때 빛 방향(왼쪽 위)으로 밝기를 정했더니 90° 돌리면 재는 면이 회색이 되어 되돌렸다. 바닥 그림자는 **재는 모서리의 반대편**으로 진다(화면 기준 오른쪽 아래로 고정했더니 90° 돌리면 짙은 그림자가 연필 대는 모서리에 깔렸다 — 사용자). 재는 모서리에는 옅은 가장자리 그늘(`srTopSh`)만
  **마킹 점은 번호가 붙은 점이다**(사용자) — 마킹 획이 그 점을 `op.p` 로 가리켜(`ptsOf` 에 들어간다) 작도 순서·번호 다시 매기기·재생·지우기에서 보통 점처럼 다룬다. 예전 마킹은 afterLoad 가 그 자리의 점을 찾아 잇는다. 점이 스케일자 모서리 위라 자 밑에 가려지므로 **마킹 점만 도구보다 위 층 `#cadPtsTop`** 에 그린다(`markPtSet`).
  **마킹**(`srMark`): 샤프로 눈금 모서리를 끌지 않고 누르면 가장 가까운 작은 눈금 자리에 **점 하나**(나중에 자를 맞추고 잇는 기준, 스냅됨)와
  **짧은 표시**(종이 위 `SR_MARK` 1.5mm·가는 선 — 3mm 에서 반으로, 사용자; `opSvg` 가 마킹은 늘 이 길이·굵기로 그려 예전 것도 따른다, 자 바깥 쪽, 점 번호 없는 두 좌표 `free` 획 + `op.mark = {s, r}`)를 남긴다. 알림에 읽은 치수를 띄우고,
  자를 옮기지 않고 같은 모서리에서 이어 찍으면 **앞 마킹과의 간격**도 보인다(`lastMark`). 마킹은 따로 된 **마킹 레이어** — 설정(⚙)에서
  보이기(`SET.markOn`, `.mark-off`)와 **모두 지우기**(`clearMarks`: 표시 + 다른 것이 쓰지 않는 마킹 점). 모서리마다 축척·0 자리·방향
  (`srS`·`srU0`·`srDu`·`srOut`)을 `srEdges` 가 싣는다
- **템플릿 계산기**(도구 줄 '계산기', `#cadCalc`): 종이 위 지름 = 실제 지름 ÷ 축척. 실제 지름(mm·m)을 넣으면 가장 가까운 구멍 번호와
  오차, 그 사이의 두 번호를 보이고, 번호를 고르면 실제 지름·반지름을 보인다. 축척은 종이 축척으로 시작하되 손으로 바꿀 수 있다.
  번호를 누르면 펴 둔 템플릿에서 그 구멍을 붉은 고리로 짚는다(`tplPoint`). 도면을 만지는 동안에는 창을 닫지 않는다
- **I자 윗날을 따라 그을 때 삼각자가 놓인 구간은 그어지지 않는다**(`triBlock`) — 밖에서 긋기 시작하면 삼각자 모서리에서
  멈추고, 삼각자 밑변 구간 안에서는 시작되지 않는다(실물처럼 연필이 삼각자에 걸린다). 아랫날·삼각자 변은 상관없다
- **모서리에서 긋기 시작하면 끄는 방향으로 변을 고른다**(`startPencil` 의 `cands` — 누른 곳 40px 안의 자의 변들 가운데 처음 5px 끈 방향과 가장
  나란한 변). 누른 곳만으로 고르면 I자 날·삼각자 세로 변이 만나는 모서리에서 세로 변이 잡혀 옆으로 끌어도 '0m' 에 머물렀다(사용자 지적)
- **긋기·붙기는 '자의 변' 목록 하나로 한다**(`guideEdges` = I자 날 둘 + 삼각자 세로 변·빗변).
  `nearestEdge` 로 가까운 변을 고르고 `onEdge` 가 변 위의 자리를 눈금에 맞춘다 — 가파른 변은 y 를,
  누운 변은 x 를 맞추고 나머지는 변에서 계산한다(60°·30° 선은 끝점이 소수 셋째 자리까지 나온다).
  샤프 긋기(`startPencil`)도 선 도구의 점 붙기(`snapXY`)도 모두 이 둘을 쓴다
- **미리보기 층(`#cadPre`)은 `pointer-events: none`** 이어야 한다. 커서 자리의 빨간 십자가 바로
  커서 밑에 있어, 없으면 모든 누름의 target 이 그 십자가 된다(I자 몸통을 눌렀는데 점이 찍혔다)
- **Ctrl(⌘)+끌기는 어느 도구든 화면 이동**이다(사용자 요청). 선택 도구에서 끌지 않고 떼면 Ctrl+누르기(더해 고르기)로 남는다
- **선택 도구로 빈 곳을 끌면 화면 이동(pan)이다**(사용자 요청, 2026-09). **영역 선택은 Shift+끌기** — AutoCAD 처럼
  끄는 방향으로 가른다: → 는 상자 안에 다 든 것만(파란 실선), ← 는 걸친 것까지(초록 점선). 상자 안의 점도 함께 고르고,
  지울 때 점은 **남은 객체가 쓰지 않을 때만** 지운다. 점 끌어 옮기기는 뺐다(사용자 요청) — 점은 점 목록에서 좌표로 고친다
- **점 번호 고치기**: 점 목록의 번호 칸을 고치면 그 번호를 쓰는 객체가 모두 따라간다
  (`REF_KEYS` = a·b·c·p·fr — 객체에 점을 가리키는 칸을 새로 만들면 여기에도 넣을 것).
  있는 번호로 바꾸면 두 점을 맞바꾼다. '번호 다시 매기기'는 작도 순서에 처음 나오는 차례로
  1, 2, 3 … 을 준다(쓰지 않는 점은 그 뒤). **이때 겹친 번호 자리도 고른다**(`placeLabels`, 사용자) — 점마다 여덟 자리(`lblBox`: 오른위·왼위·오른아래·왼아래·오른·왼·위·아래) 가운데 이미 놓인 번호·점·선과 덜 겹치고 **제 점보다 다른 점에 더 가깝지 않은** 곳을 붐비는 점부터 고르고 `p.lp` 에 적는다(없으면 예전 오른위 자리). 번호가 이미 차례대로여도 자리는 고른다
- **호 도구의 '모서리 라운드'**(필렛): 두 선이 만나는 모서리를 누르면 두 선을 접점까지
  자르고 접하는 호(`fr` = 모서리 점)를 넣는다. 호는 그 선의 선 종류·굵기를 따른다.
  모서리는 **좌표로 찾는다** — 사각형은 마주 보는 두 꼭짓점만 점으로 가져 나머지 둘에는
  번호가 없기 때문이다. 사각형 모서리를 깎으면 사각형을 네 선으로 풀어 둔다. 결과는
  AutoCAD FILLET 처럼 보통 선·호라, 모서리 점을 나중에 옮겨도 호는 따라가지 않는다
- 선 굵기·점·글자 크기는 **종이 폭 100m 기준 값에 `U`(종이 폭/100)를 곱한다** —
  1:20 단면도(2m)와 444(170m)가 같은 굵기로 보인다
- 기호를 넣으면 그 모양을 도면(`doc.syms`)에 복사해 둔다 — 라이브러리에서 지워도
  도면은 그대로다. 기호 등록 = 객체를 고르고(Shift) 기준점 번호를 주는 것(AutoCAD BLOCK)
- **SVG 내려받기는 작업형 화면의 단계 재생 꼴이다** — `data-steps` + `<g data-step>` +
  `<g data-sub>`. 작도 순서의 '단계'가 (1)(2)… 가 되고 그 안의 객체 하나가 한 컷이다.
  점은 처음 쓰인 컷에 함께 나타난다. '작도 순서 복사'는 점 좌표표 + 작도 순서 글을
  워드에 붙일 꼴로 복사한다
- **글자 각도**(`#ftAng`, 도구 줄 가로·세로 옆) — 0° · +45°(시계) · −45°(반시계). 고르면 글자 칸의 '회전'(`ftRot`)에 들어가고 쓰는 동안 미리보기도 기운다. 기억(`SET.ang`)(사용자)
- **글자 도구 커서는 '└ 가' 모양**(`.cad-tool-text #cadSvg`, SVG data URI — 홑따옴표·`%23`), 붉은 점(hotspot 3,25)이 첫 글자의 왼쪽 아래 모서리다
- **글자 도구는 도면에 바로 쓴다**(`openTextBox`·`drawTextPre`) — 입력칸은 투명·2px 로 키와 한글 조합만 받고(누른 자리에 두어
  후보창이 곁에 뜬다), 쓰는 글자는 넣을 때와 같은 `textSvg` 로 `#cadTxtPre` 에 그리며 끝에 깜박이는 빨간 커서를 둔다. Enter 로 넣는다(Esc 그만두기, 다른 곳을 누르면
  쓴 것을 넣는다). **글자는 점을 찍지 않고 좌표(`op.x`·`op.y`)를 직접 갖는다** — 그 좌표가 **첫 글자의 왼쪽 아래 모서리**이고 가로는
  오른쪽으로, 세로(`op.dir='v'`, `writing-mode:vertical-rl`)는 아래로 써진다. 글꼴마다 잉크가 글자 칸 안에 놓이는 자리가 달라
  실제로 그려 잰 비율(`INKOFF`, 글자 크기 대비)만큼 옮겨 그린다 — 기준선만 바꿔서는 고딕이 0.1칸 떠 보인다. 옛 글자(`op.p`)는 점을 쓰고 가운데
  맞춤 그대로(`textPos`). 가로·세로, 글꼴(고딕·명조·손글씨, `op.font` → `FONTS`), 크기는 도구 줄 T 옆. **크기는 레터링 템플릿처럼 종이 위 mm 로 고른다**(`#ftMm` 2~10mm, 기본 3mm, localStorage `cadFtMm`) — 도면 m 는 `mm × 축척/1000` 으로 셈해 감춘 `#ftSize` 에 둔다(`ftMmSync`). m 숫자 칸은 '0.525' 처럼 끝수가 잘려 보기 싫다는 지적에 뺐다. 목록에 없는 크기의 옛 글자를 고르면 그 mm 를 목록에 더한다(`ftMmFrom`). 나눔 손글씨는 글자가 작게 그려져 1.35배로 키운다
- **보조선은 따로 된 레이어다**(`isAux` = `ls==='aux'`). 맨 아래에 먼저 그리고(`g.aux`), 도구 줄 '보조선' 단추·설정에서 켜고 끈다
  (`.cad.aux-off`). 꺼진 보조선은 지우개·영역 선택이 건드리지 않고 SVG 내보내기에서도 빠진다(보이면 `data-layer="aux"`).
  색은 설정(`SET.auxColor`, 기본 #a3a9b3). 꺼 둔 채 보조선을 그으면 레이어를 켠다
- **스냅 켜기·간격은 도구 줄에 있다**(한때 설정 창에 넣었다가 자주 바꾼다고 되돌렸다. 값은 그대로 `cadSet` 에 남는다)
- **프리드로잉**(도구 `free`, 단축키 D): 누른 채 끌면 2px 마다 한 점을 모아 `op.pts`(좌표 목록, 점 번호 없음)로 둔다.
  떼면 자가 덮은 곳(`covered`)에서 끊어 조각마다 한 획으로 넣고, 떨림은 0.6px 더글러스-포이커로 거른 뒤 중점 2차 곡선으로
  매끄럽게 그린다(`freeD`). 샤프(0.5·0.9)·선 종류는 오른쪽 칸(`ffW`·`ffLs`), 커서는 고른 샤프 것을 빌린다(루트에 `cad-tool-pen5/9` 를 함께 단다).
  지우개는 촘촘히 다시 찍어 원판 안을 빼고 조각으로 나누고(`cutFree`), 채우기의 벽에도 들어간다. 자 몸통을 누르면 옮기기
- **도형 채우기**(도구 `fill`, 단축키 F, `fillAt`): 누른 면을 설정의 채우기 색·진하기(`SET.fillColor`·`fillOp`)로 칠한다.
  종이를 긴 변 1600칸 격자로 놓고 보이는 선·사각형·원·호를 2.2칸 굵기의 벽으로 canvas 에 그린 뒤 누른 칸에서 4방향으로 번진다.
  **종이 가장자리에 닿으면 면이 막혀 있지 않은 것 — 경고만 하고 칠하지 않는다.** 칠한 칸은 벽 쪽으로 2칸 넓혀(선 밑 흰 틈 방지)
  칸 경계를 고리로 잇고(evenodd 라 잇는 순서와 상관없이 구멍이 맞다) 더글러스-포이커(0.75칸)로 줄여 `op.d` 경로로 저장한다.
  **비연관**이다 — 선을 나중에 옮겨도 칠은 따라가지 않는다. 칠은 맨 아래 층, 작도 순서의 ✕ 로 통째로 지우고(✎ = 지금 색으로 다시 칠함),
  **지우개로 문지른 자리만 지울 수도 있다**(사용자 요청) — 원판을 `op.cuts` 에 쌓고 그 원들을 검게 뚫은 `<mask>` 로 가린다(`fillTouch` 가
  원판 가운데·둘레 16점을 `isPointInPath` 로 짚어 닿았는지 본다, 자가 덮은 곳은 빼고). 경로 자체는 그대로라 채우기 벽 계산에는 영향이 없다.
  면적 오차: 1:430 에서 원 58.1㎡ → 58.5, 정사각형 89.5 → 90.2
  **채우기 모양은 도구 줄 채우기 아이콘 옆 그림 단추**(`#cadFpBtn`, 지금 모양을 그림으로; 누르면 여덟 그림 `#cadFpMenu`)에서 고른다 —
  설정 창에 두었다가 옮겼다(사용자 요청). 상태는 감춘 `#cadFillType` select 가 들고 `SET.fillType` 에 남는다. 그림은 `fillSwatch(k)`
  **채우기 유형**(`SET.fillType` = solid·h·v·d(사선 ／)·x(X자 = 두 사선)·dot(점, 줄마다 반 칸 엇갈림, 반지름 = min(0.28U, 간격×0.13)) · gl(큰 가로세로 격자, 간격 1.5배) · grass(잔디 — 가운데 곧은 잎 + 밑동을 조금 띄운 좌우 긴 잎이 위에서 바깥으로 휜 풀포기를 줄마다 엇갈려, 자리·키를 sin 해시로 늘 같게 흔든다. 좌우 잎이 짧고 한 점에서 나오면 ↓ 화살표로 보였다) · gs(작은 격자, 기본 간격 그대로 — 절반 → 0.75배 → 1배로 두 번 '더 넓게' 요청, 굵기 min(0.11U, 간격×0.16)) — X자는 선 굵기 절반): 수평선·수직선은 면 경로를 clipPath 로 삼아 테두리 상자(`op.bb`) 전체에
  평행선을 긋고 오려 낸다. 간격(`op.gap`)은 만들 때 면의 짧은 쪽 ÷ 8 을 0.1·0.2·0.25·0.5·1·2·2.5·5 … 로 반올림 — 면이 커지면 넓어진다.
  선·점은 채우기 색을 **연한 펜처럼 진하기 0.35~0.6** 으로 긋는다(처음 최소 0.85 였다가 '조금 연한 펜으로' 요청). 굵기는 0.14U 와 **간격의 1/5** 중 작은 값(X자는 두 겹이라 그 절반 — 0.07U·간격의 1/10) — 좁은 면에서 선이 뭉쳐 면처럼 보였다
- **점 보이기는 도구 줄 '점' 체크**(`#cadShowPts`, 스냅 왼쪽 — 설정에서 옮겨 왔다). **설정(도구 줄 오른쪽 끝 톱니)**: **점 크기·번호 크기**(배율 25~300%, `SET.ptSize`·`ptNum` → `dotSvg`), **독립 점 제거**(`dropLonePoints`: 어떤 객체에도 쓰이지 않는 점을 확인 후 지우고 번호를 메운다 — 스케일자 마킹 점은 남긴다), 삼각자 크기(도면 m 환산 표시). **샤프 0.9·0.5 선 굵기**는
  종이 위 mm(기본 0.7·0.4 — 1:430 에서 예전 굵기와 같다)로 받아 `mm/1000 × 축척`(m)으로 그린다(`stAttr` 의 p9·p5). 이미 그은 샤프 선도 따라 바뀐다. **샤프마다 선 종류도 설정에서 정한다**
  (`SET.ls9`·`ls5`, 기본 0.9 실선 · 0.5 보조선 — 오른쪽 샤프 칸 `fnLs` 와 같은 값). 샤프 선의 굵기는 선 종류(보조선 포함)보다 설정 mm 가 먼저다. 도구 줄에서 옮겨 왔고
  localStorage(`cadSet`)에 남는다. 삼각자 크기는 삼각자를 치워도 기억해 다음에 꺼낼 때 쓴다
- **도구 줄은 그림이 있으면 그림만 보인다**(공간 절약, 사용자 요청) — I자·삼각자·템플릿·계산기·스케일자·보조선·샤프의 이름 글자를 뺐고 이름은 `title`·`aria-label` 에 둔다. 샤프 0.9 는 노랑, 0.5 는 검정 그림으로 가른다. **글자 글꼴은 설정(⚙)으로 옮겼다**(`#ftFont`, `SET.font`). **I자 펴기·치우기도 설정으로 옮겼다**(체크 `#cadBarOn` — 도구 줄 단추는 없앴다; 샤프를 고르면 여전히 저절로 편다)
- **CAD 탭을 누르면(또는 `?tab=cad` 로 열면) 바로 전체 화면**이다(`setFull`). ⛶·Esc 로 빠져나오면 탭 안의 보통 크기로 돌아간다
- **도구 줄 '점' 옆에 '보조선'·'마킹' 체크**(`cadAuxChk`·`cadMarkChk`, 사용자) — 설정 창의 보이기와 같은 것(`setAux`·`setMark` 가 서로 맞춘다). 보조선 그림 단추(`cadAuxBtn`)는 감췄다. 삼각자 뒤집기 단추는 ⇆ 그림만, 스케일자 종류의 사용자 지정은 '1:N'
- **작도 순서 목록에서 마킹 줄은 녹색, 보조선 줄은 빨강**(`.cad-op.mk`·`.ax`, 사용자)
- **작도 순서 번호는 CSS 동그라미**(`.cad-op .no i`) — 글꼴 원문자는 ⑳ 뒤가 (21) 로 바뀌었다(사용자). 복사·재생 글에 쓰는 `circ` 도 ㉑~㊿ 까지 원문자다.
  **선 줄에 길이(m)**를 적는다 — 보조선은 빼고(사용자)
- **두 줄(경계석)**(`#cadDbl`, `makeDouble`, 사용자) — 고른 선에 간격을 두고 한 줄 더 — **간격은 누를 때마다 뜨는 작은 창에서 사용자가 정한다**(기본은 마지막에 쓴 값, 처음 0.2m; Enter 로 긋기). 닫힌 것(사각형·원·끝과 끝이 이어져 닫힌 선들·닫힌 프리드로잉)은 **안쪽**(면적 부호로 안쪽을 가린다), 열린 것은 쪽을 고른다(첫 변이 가로면 위/아래, 세로면 왼쪽/오른쪽, 호는 안쪽/바깥쪽). **선과 호를 끝과 끝으로 이어 한 사슬(고리)로 모은다**(`dblTargets` — **가장 가까운 끝끼리 0.6m 안이면** 잇는다; 손으로 그린 둥근 모서리는 호가 직선 끝에서 0.2~0.4m 겹치거나 떨어져 있어(주차공원 444) 0.002·0.05m 로는 열린 것으로 잡혔다, 사용자. 이음매는 안쪽 선끼리의 교점 — 선·선은 직선 교점, 선·호는 직선과 원, 호·호는 두 원 — 원래 이음매에 가장 가까운 것으로 자르거나 늘인다; 둥근 모서리가 낀 윤곽을 따로 처리했더니 안과 밖이 섞였다, 사용자). 고리의 안쪽은 면적 부호로 한 번에 정하고(호는 점을 뿌려 넣는다), 선은 법선으로·호는 반지름을 늘이거나 줄여 평행이동, 이음매는 선끼리면 교점·호가 끼면 호 끝점. 열린 사슬은 꺾이거나 휘었으면 **안쪽/바깥쪽**(양 끝을 이은 현으로 닫은 모양의 안쪽), 곧은 선 하나만 위아래·좌우 — 고른 쪽을 사슬 전체에. 새 선은 원래 선의 선 종류·굵기. **새 선마다 손으로 그었다면 쓸 도구 자리를 계산해 `tl` 에 적고**(`toolsForLine` — 가로선은 I자 윗날을 그 높이에, 세로선은 I자를 선 아래 끝에 두고 삼각자 세로 변을 그 x 에, 45°·60°·30° 선은 그 삼각자 빗변을 선에 맞춘다; 다른 각도는 지금 자리; **호·원은 `toolsForCircle` — 지름이 가장 가까운 원 구멍의 중심을 호의 중심에 맞춰 템플릿을 놓는다**, 종이 위 1mm 넘게 어긋나면 지금 자리), **만든 즉시 그 부분을 재생해 보인다**(`RP.autoEnd` — 끝나면 도구는 재생 전 자리로). 변마다 도구가 옮겨 가야 해서 묶지 않는다(템플릿 도형과 다르다)
- **템플릿으로 한 번에 그린 다각형은 한 묶음**(`op.grp`·`gname`, 사용자 — 변마다 따로 포커스를 받았다) — 변은 여전히 선 줄들이라 지우개·접선·채우기는 그대로 쓰고, 작도 순서·작도 순서 복사·재생·형광펜·지우기·↑↓·끌기·⇄ 가 묶음을 한 줄로 다룬다(`grpSpan`·`grpOps`·`grpDesc`: '정삼각형 P1 → P2 → P3 → P1 · 한 변 12m'). 재생은 묶음 가운데에서 멈추지 않고 변을 차례로 긋는다. 원은 원래 한 줄. `addPt` 는 저장값(0.001m 반올림)끼리 견준다 — 날값으로 견주어 닫힌 도형의 끝점이 시작점과 다른 번호로 생겼다. 이 기능 전에 그린 도면은 서버에서 묶었다(같은 `tl` 도장 = 한 번 돌린 변, 주차공원 444 23묶음). **불러올 때도 브라우저가 묶는다**(`groupTplOps`, afterLoad) — 브라우저에 남은 옛 작업본(초안)은 서버를 고쳐도 그대로라 여러 번 반짝였다
- **⇄ 긋는 방향 바꾸기**(`revOp`, 선·호·프리드로잉 줄, 사용자) — 선은 두 점을 맞바꾸고, 호는 도는 방향도 뒤집어 같은 호로, 프리드로잉은 점 차례를 뒤집는다. 바꾼 방향을 형광펜으로 보인다. 재생의 샤프·도구 자리는 그대로
- **작도 순서 줄은 마우스로 끌어 옮긴다**(HTML5 draggable, 사용자) — 놓일 자리에 녹색 줄, 가장자리로 끌면 목록이 저절로 내려간다. **단계 줄을 끌면 딸린 줄(다음 단계 전까지)이 묶음으로 함께 옮겨지고**(`opGroupOf`), 다른 묶음의 앞·뒤에만 놓인다 — 가운데에 놓으면 그 단계가 쪼개진다. Alt+끌기는 제목 줄 하나만(단계 경계 옮기기). 재생 중에는 안 된다
- **작도 순서 줄을 눌러 고른 것은 도면에서 빨갛게 칠하지 않는다**(`listSel`, 사용자) — 형광펜·동그라미가 알린다. 고른 상태는 남아(목록 줄 분홍, '+ 단계'·지우기에 쓰인다) 도면을 만지면 평소 선택 표시로 돌아간다
- **단계 이름 칸 · + 단계 · ▶ 재생은 한 줄**(`.cad-steprow`)이고 **목록을 내려도 위에 붙어 있다**(sticky, 사용자). 단계 줄은 연녹 바탕
- **'+ 단계' 는 작도 순서에서 마지막으로 누른 줄(`opPick`) 바로 위에 단계를 넣는다**(재생 중에 누른 줄도 — 그때는 selOps 가 안 바뀌어 옛 선택 줄 위로 들어갔다; 없으면 고른 줄 가운데 가장 앞, 그것도 없으면 맨 끝 — 사용자).
  **재생 중에도 목록의 ✕·✎·↑↓ 가 듣는다** — 재생을 끝내고 한다(재생 처리가 먼저 가로채 아무 일도 안 했다). 불러올 때 `doc.seq` 를 가장 큰 id 보다 크게 바로잡는다(id 가 겹치면 한 줄 지울 때 다른 줄도 지워진다). 가운데에 넣으면 목록을 맨 아래로 내리지 않고 그 단계 줄을 보인다
- **목록의 선 줄은 '실선 P1 → P2 · 40m' · '보조선 P3 → P4'** 꼴 — 선 종류가 앞, 굵기(샤프 0.9 등)는 안 보인다(사용자). 마킹 줄은 '마킹 P5 · 27m' — 스케일(1/800)도 뺀다 다른 줄도 굵기는 빼고 선 종류 이름만
- **작도 순서 목록·재생 글에는 점 번호만** 보인다(`desc(op)` — 좌표·I자·삼각자·스케일자 자리는 속에만 둔다, 사용자). 좌표와 도구 자리(`tlText`)까지 쓰는 것은 '작도 순서 복사'(`desc(op, true)`)뿐
- **도면에서 새로 그리면 작도 순서 칸이 맨 아래로 내려간다**(`changed` 의 `opsSeen`, 사용자) — 방금 더한 줄이 보이게. 줄 수가 늘 때만이고(지우기·고치기는 그대로), 창 전체가 아니라 목록 칸(`.cad-pane`)만 내린다. 재생 중에는 안 한다
- **작도 순서의 ✕ 지우기는 점까지 지운다**(`delOpClean`, 사용자) — 그 선이 쓰던 점 가운데 이제 아무도 안 쓰는 점과 마킹의 점을 함께 지우고 번호를 메운다(`compactPts`). 전에는 선만 사라지고 점·번호가 도면에 남았다
- **작도 기록·재생**(사용자: "삼각자·I바·스케일바의 움직임도 기록해 나중에 복귀해 볼 수 있게"): 선·원·글자를 넣을 때마다 그 순간의
  I자·삼각자·템플릿·스케일자 자리를 `op.tl` 에 적는다(`changed` → `stampTools`, `toolState`). '작도 순서 복사'에만
  "I자 Y30 · 45° 삼각자 X78 · 스케일자 1/300" 꼴로 붙는다(`tlText`, 목록에는 안 보인다). 기록 전에 그린 것은 `tl = 0`(afterLoad) — 재생 때 도구를 건드리지 않는다
  - 작도 순서 칸의 **▶ 재생**(`rpStart`) → **작도 순서 탭 위** 조작 막대(`#cadRp`, 사용자 — 도면 위에 띄우면 종이를 가렸다; 재생을 켜면 작도 순서 탭으로 바꾼다): ⏮ ◀ ▶재생/멈춤 ▶ ⏭ · 빠르기 · 끌어 옮기는 막대(앞뒤로 끌면 그 칸까지 그려진 모습 — 핸들러에서 값을 먼저 읽어야 한다, rpStop 의 rpInfo 가 막대를 되돌린다) · 닫기.
    한 칸 나아갈 때 도구를 앞 자리에서 새 자리로 **움직여 보이고**(`rpTween`, 같은 도구의 숫자만 잇는다) 그 선을 **긋듯이** 보인다
    (`rpReveal` — rAF 로 dashoffset 을 늘리며 **샤프 그림이 선 끝을 따라간다** — 선마다 시작·끝은 천천히, 가운데는 빠르게(easeInOutCubic, 시간은 길이에 비례해 선마다 나눔, 사용자). 스케일자 마킹(`op.mark`)만은 처음부터 고른 속도로, 빨리(긋는 시간이 보통 선의 약 30%) 긋되, 다음 칸으로는 넉넉히 쉬었다 넘어간다(마킹 뒤 0.5 · 보통 선 뒤 0.15 × 빠르기): 커서로 쓰는 실사 샤프 PNG 를 스타일시트의 `.cad.cad-tool-pen9/5 #cadSvg` cursor 에서 읽어 `#cadRpPen` 에 화면 px 크기로 놓고 샤프심 끝(hotspot)을 선 끝에 맞춘다, 샤프 0.5 선이면 0.5 그림. 칠하기·글자는 샤프 없이 서서히 나타난다). 지금 선은 제 색 그대로 긋는다(빨강으로 긋고 다음 칸에서 검정으로 덧칠되던 것을 뺐다, 사용자), 목록의 그 줄은 노랑(`.cad-op.rp`)
  - **다음에 그을 자리 표시**(`rpMark`, 사용자 제안 — 형광 노랑 rgba(255,245,0,.5) 바탕(형광펜 띠와 같은 색, multiply) + 검은 테두리 동그라미, 테두리는 1.5px·불투명도 60% — 빼면 노란 삼각자 위에서 묻힌다): 한 칸 나아갈 때 시작점에
    반지름 60px 에서 14px 로 **조여 들며** 나타나 한 번 반짝이고 사라진다(퍼지는 원은 눈이 바깥으로 따라가고, 조여 드는 원은 한가운데로 모인다).
    도구가 옮겨 가는 동안 떠 있어 샤프가 그 자리에서 긋기 시작한다. 시작점이 화면 밖이면 그 자리로 화면을 옮긴다. 시작점은 `rpStartPt`
    (첫 점 번호 → 프리드로잉 첫 좌표 → 글자 좌표 → 칠한 면 가운데). **작도 순서의 줄을 눌러도 같은 동그라미가 뜬다**(사용자, 재생 밖에서도 — 단계 줄이면 그 단계의 첫 작도). 표식 토큰은 `rpMark.tok`. 이어서 **노란 형광펜**(`rpHilite`, 선 밑 층 `#cadHl`)이 시작점에서 끝점까지 그 선을 0.45초에 쓱 칠하고 잠시 머물다 사라진다 — 어느 선인지와 긋는 방향이 함께 보인다. 형광펜 노랑 rgba(255,245,0,.5)·폭 화면 9px, 층에 `mix-blend-mode: multiply` 라 모눈이 비친다(처음 14px 짙은 노랑은 "너무 굵다"). 쇄선도 이어진 띠로. 원·호는 둘레, 사각형은 네 변(쇄선이면 변마다), 글자·기호·칠한 면은 둘레 상자가 반짝(사용자 제안에 따라)
  - 재생 중에는 도면을 누를 수 없고(휠 확대·휠 버튼 이동만) 단축키도 막는다. 키: ←→ · Space · Home/End · Esc. 목록 줄을 누르면 그 칸으로 간다
  - 끝내면(`rpEnd`) 재생 전 도구 자리로 되돌린다. `RP` 는 `var` — 초기화 중 renderAll 이 먼저 불려도 TDZ 가 안 나게
- 저장 전 작업은 localStorage(`cadDraft:<cert>`)에 늘 남는다
- **고른 것의 빨간 표시는 원래 테두리(`stroke` 속성)가 있는 요소에만 입힌다.** 글자·칠한 면에 stroke 를 입히면 굵기가 기본 1(= 도면 1m)이라 글자를 덮는 빨간 덩어리가 됐다(사용자). 글자는 글자색만 빨갛게
- 포인터 캡처를 쓰므로 **떼는 순간의 `e.target` 은 늘 svg 다** — 누를 때 target 을
  적어 두었다가 쓴다(선택 도구가 객체를 못 고르던 원인)

## 블렌더로 도면을 세운다 — `make428.py` · `make401.py`

치수가 맞아야 하는 그림(입체 평면도·축측도)은 **이미지 모델로 만들지 않는다.** 화면에서
길이를 재지 못해 "주차 10대"·"연못 ⌀4m"를 몇 번 말해도 회차마다 달라진다(401 에서 22회차를
태우고 접었다). 블렌더 스크립트는 **숫자를 고치면 그 자리만 정확히 바뀐다.**

```bash
blender -b -P make428.py -- --out r01.png            # 축측도(45° 평행투영)
blender -b -P make428.py -- --out t01.png --top      # 평면 렌더 — 도면과 1:1 대조용
blender -b -P make428.py -- --out b01.png --bird     # 조감도(원근)
blender -b -P make428.py -- --out m.png --max --threads 14   # 5120px·1024샘플
```

- 파일 맨 위 **DIMS 한 곳만** 고친다. 좌표는 도면과 같은 단위(m)이고
  `bl(x, y)` 가 블렌더 좌표(Y 뒤집기)로 바꾼다
- **401 은 좌표를 다시 읽지 않았다** — `make_grid401_svg.py` 의 미터 좌표를 그대로 옮겼다.
  428 에서 가장 오래 걸린 일이 도면을 눈으로 읽는 것이었는데 그 일이 이미 끝나 있었다
- 401 은 **표고가 절대값**(71~72m대)이고 등고선에서 높이면을 만든다(IDW).
  계획고: 북동 진입광장 72.2 · 목재데크광장 72.0 · 남동 진입광장 71.6 · 마운딩 72.6.
  **수면은 도면에 없어 가정**했다(저수지 70.8 · 습지 71.0 · 실개천 70.9)
- 검증은 눈대중이 아니라 **겹쳐 보기** — 도면의 검은 선만 뽑아 1m 를 같은 화소로 맞춰
  평면 렌더 위에 빨갛게 얹고, 어긋난 곳의 숫자를 고쳐 다시 렌더한다

### 도면은 **부지 좌표로** 읽는다 — `read_401_drawing.py`

사용자가 화면 조각을 붙여 주더라도 그것으로 읽지 말고 `_ls_drawing_refs/<번호>_성운/*.jpg`
원본(2400~2500px)을 연다. 붙여진 조각은 200px 남짓이라 인출선이 안 보이고, 그걸로 읽으려다
"2m 식재 띠가 있다" 같은 오독을 냈다.

```bash
python read_401_drawing.py planting 64 92 44 61 out.png   # 부지 x64~92 · y44~61 (m)
python read_401_drawing.py plan     -22 70 24 46 lead.png # 음수 x 로 라벨 블록까지
```

**축척은 눈대중으로 잡지 말고 경계선을 화소로 재서 잡는다.** 401 은 두 시트가 모두
**12.89 px/m** 였다(배식설계도 원점 538,279 · 기본설계도 400,468.5). 눈대중으로 잡았을 때는
y 가 7% 어긋나 남측 경계를 58m 로 읽었고, 그래서 "철쭉 띠가 57.6 에서 끝난다"고 본 것이
실제로는 경계선(60)까지 내려오는 띠였다.

- 경계선 찾기: 부지 범위 안에서 **60% 이상이 검은 가로·세로 줄**을 훑는다.
  도곽선이 함께 잡히므로 범위를 부지 안으로 좁힐 것
- **수량표는 마지막에 대조한다.** 수종을 자리로만 잡으면 수가 어긋난다 — 보행로 서쪽 열을
  느티나무로 넣어 두었더니 느티나무가 12주가 됐다. 남측 라벨 인출선을 따라가니 그 열은
  **메타세쿼이아 7주**였고, 바로잡으니 메타세쿼이아 5+7=12 · 느티나무 5+2=7 로 딱 맞았다.
  **수량이 맞아떨어지는 것이 자리를 제대로 잡았다는 신호다**
- 관목을 띠에 그냥 뿌리면 수량이 어긋난다(찔레 140 자리에 **854주**). 수량표 값에서 멈출 것
- 두 시트를 견줄 때는 **같은 축척·같은 창**으로 뽑아 나란히 붙인다. 401 의 기본설계도와
  배식설계도는 광장 왼쪽 변 82 · 동측 경계 90 · 보행로 서측 73 이 모두 일치했다

### 세 번 걸린 함정

- **`--bird` 가 `--max` 의 해상도를 되돌린다.** 인자 처리 차례 탓에 `--bird --max` 가 조용히
  1920×1280 을 뱉었다. 4분이면 끝나 버려 눈치채기도 어렵다(5120 이면 20분이 넘는다).
  지금은 `SAMPLES` 가 이미 잡혀 있으면 RES 를 건드리지 않는다
- **포장 판이 통째로 사라진다.** `slab(x0,x1,y0,y1,z,m,h)` 의 `z` 는 **윗면**이다.
  `z-h` 를 넘기면 포장이 h 만큼 잠겨, 지형(계획고−6cm)에 덮여 안 보인다
- **지형과 포장을 같은 높이에 두면 Cycles 에서 새까맣다**(EEVEE 는 덮어 가려 못 알아챈다).
  지형을 6cm 내리고 판마다 z 를 조금씩 어긋나게 둔다
- **띠(관찰로)를 바깥선+안선 한 ngon 으로 묶으면 안 된다.** 삼각분할이 even-odd 를 안 써서
  가운데까지 메워지고 법선이 뒤집혀 새까맣다. 두 선을 **길이 비례로 짝지어** 사각형을 잇는다
- **원통은 눕히는 회전을 빼먹기 쉽다.** 블렌더 원통은 Z 축이 기본이라 길이 40m 짜리가
  그대로 솟는데, **위에서 내려다보는 평면도·축측도에서는 점으로만 보여 안 보인다.**
  같은 까닭으로 428 동측 완충녹지가 경계에서 떨어져 있던 것도 조감도에서야 드러났다 —
  **모형을 고쳤으면 각도를 바꿔 한 번 더 본다**

## GPT 이미지 모델 다루는 법 (OpenAI / Gemini)

쪽집게 노트 **인포그래픽**, 조경기사 실기의 **조감도·입체평면도**를 모두 이 모델로 만든다.
도구와 검증 요령은 아래에 공통으로 적고, 도면별 경위는 [docs/조경기사.md](docs/조경기사.md).

**먼저 읽을 것 — 세 가지가 이 작업의 전부다.**

1. **화법**: 고칠 항목마다 먹히는 말이 다르다. 엉뚱한 화법을 쓰면 회차만 태운다
2. **덧그리기**: 화질을 깎는 것은 수정 항목 수가 아니라 **덧그린 횟수**다
3. **검증**: 만들고 끝내지 말고 **Claude 가 눈으로 + 화소로** 재고, 기준 미달이면 버린다

### 1. 화법 — 항목마다 먹히는 말이 다르다

401 입체평면도·조감도를 22장 그리며 갈린 것이다. **모델은 화면에서 비율을 재지 못한다.**

| 고칠 것 | 안 먹힌 말 | 먹힌 말 |
|---------|-----------|---------|
| 투영·시점 | "원근 없이" · "30° 기울여" | **재서 확인되는 조건** — "부지가 사다리꼴이 아니라 **평행사변형**" · "도로 띠 폭이 위아래로 **같다**" · "주택이 모두 **같은 크기**" · "깊이 비율 **0.71**" |
| 배치 | "부지 폭의 12% 지점" 같은 **백분율**(3연속 실패) | **"무엇을 지워라 / 늘려라"** — "관찰로 서쪽 열린 땅의 나무를 **모두 지우고** 빈 잔디로" |
| 개수 | "안내판은 3개여야 한다"(2연속 실패) | **자리를 짚고 세게 한다** — "데크 난간이 꺾여 끝나는 **모서리 바로 오른쪽**에 선 흰 판 액자를 지우고, **바로 아래 두 개와 똑같이 생긴** 걸상을 놓아라. 세어서 하나, 둘, 셋" |
| 빠진 식재 | — | **"가장자리를 따라 한 줄로 빈틈없이** 심어라, 끝에서 꺾어 아래 변도" |
| 수종 | 이름만 적기(참나무 셋은 그래도 다 같게 나온다) | **실루엣이 다른 것만** 이름을 적는다 — 은행나무(좁고 곧은 황록)·버드나무(늘어진 장막)·메타세쿼이아(뾰족한 원뿔) |
| 빈 잔디 | 말 안 하면 모델이 나무로 채운다 | **"열려 있어야 한다 · 정확히 N그루 · 세어라 하나, 둘, 셋…"** + 옆 화단을 "넓은 화단"이 아니라 "폭 3m 좁은 띠"로 |
| 지우면 안 되는 것 | (말 안 하면 사라진다) | **"관찰데크 5 + 조류관찰소 1 은 지우지 마라, 여섯이 모두 남아야 한다"** 를 따로 못박는다 |

- **자리는 붙박이 지형지물로 짚고, 개수는 끝까지 센다.** 세 번 다 이것으로 풀렸다 —
  모임광장 평의자 3(4회 실패 뒤), 진입광장 안내판 3(2회 실패 뒤), 메타세쿼이아 7주(**4회 실패 뒤**).
  좌표·백분율·"오른쪽 변을 따라"는 모두 소용없었다. 쓰는 법은 둘이다:
  ① **그림에 이미 있는 물건**을 기준점으로 — "나무가 심긴 네모 **수목보호대 두 개**가 박힌
     포장을 찾아, 그 **옆에서 시작해** 포장 가장자리를 따라 북으로"
  ② **"세어서 하나, 둘, 셋, 넷, 다섯, 여섯, 일곱"** 을 **끝까지 적는다**. "일곱 그루"만으로는 안 된다
  ③ 그래도 수가 틀리면 **틀린 경우를 짚어 준다** — "다섯 개다" 보다 **"셋이 아니다. 셋째가
     생기면 빼고 그 자리에 포장을 깔아라"** 가 들었다(수목보호대가 2 → 3 으로 늘던 것)
- **고치는 지시는 범위를 좁혀라.** "포장 가장자리의 키 큰 침엽수를 **모두** 지워라"가 통해서
  **맞게 서 있던 메타세쿼이아 열식까지 지웠다.** "무엇을 지워라"는 세면서 그만큼 난폭하다 —
  **"마운딩 옆 동선 가장자리의"** 처럼 자리를 한정해 쓴다
- **없어지면 안 되는 것은 매번 다시 적는다.** 한 회차에서 수목보호대가 5 → 3 으로 줄었다.
  "수목보호대는 **모두 다섯 개**다 — 남측 광장 둘, 북동 광장 둘, 모임광장 하나. 하나도 없애지
  마라" 를 넣으니 남았다
- **이미 맞게 그려진 것에 맞추게 한다** — "바로 아래 두 개와 똑같이 생긴". 새로 정의하는 것보다 쉽다
- **두 요구가 부딪히면 모델은 조용히 한쪽을 버린다 — 어느 쪽이 우선인지 적어라.** 안내판을
  "경계선과 나란히" 두면서 "판면이 비스듬히 보이게"까지 적었더니, 판면을 보이려고 **방향을
  되돌려** 버렸다(경계선과 나란하면 그 시점에서 판면은 안 보이는 것이 정상이다). 부딪히는
  요구를 빼고 **"방향이 최우선, 판면이 안 보여도 괜찮다"** 로 고쳐 잡았다. 그러고는 '안내판처럼
  보이게'를 판면이 아닌 **다른 단서**(다리 둘 · 판 아래 빈 틈 · 바닥의 그림자)로 옮겼다 —
  **원하는 것을 직접 말할 수 없을 때는 그것이 드러나는 부수 단서를 적는다**
- **실사를 주문하면 원근으로 되돌아간다.** 질감만 바꾸는 회차에도 위 표의 투영 조건문을
  **같은 프롬프트에 통째로 남기고**, 회차마다 화소로 재서 확인한다

### 2. 덧그리기 — 횟수가 화질을 깎는다, 항목 수가 아니다

같은 그림을 `--ref` 로 고쳐 그릴 때마다 물러진다. **항목을 몇 개 고치느냐는 상관없다.**

| 401 My 조감도 | 방식 | 선명도(라플라시안 분산) |
|---|------|------|
| 7회차 | 원본에서 **넷을 한 번에** | 1900 → 1781 (**−6%**) |
| 8·9회차 | 그 위에 **한 가지씩 더** | −23% · **−25%** |
| 10회차 | 원본에서 **다섯을 한 번에** | 1900 → 1835 (**−3%**) |

- **고칠 게 쌓이면 모아서 원본으로 돌아가 한 번에 간다.** 한 가지만 더 고치려다 25%를 잃었다
- **`--quality xhigh`($0.074)가 듣는 그림과 안 듣는 그림이 있다.** 입체평면도(회당 −6~10%)는
  `high` → `xhigh` 로 1015 까지 회복됐지만, 이미 6회 덧그려진 조감도는 `xhigh` 로도 −25% 를
  못 막았다. **원본에 남은 여유가 다르다** — 새 그림은 `high` 로 충분하고, 덧그린 그림을
  마무리할 때만 올린다
- **열화는 프롬프트로 못 막는다.** "어느 부분도 앞 회차보다 흐려지면 안 된다"를 박아도
  1125 → 942 에 그쳤다(회복 실패)
- 선화(모눈 도면 등)를 참고 그림으로 함께 주면 더 물러진다 — 뺐더니 −16% 가 −13% 로 줄었다

### 3. 검증 — 화소로 재고, 기준 미달이면 버린다

**회차마다 Claude 가 직접 연다.** 사람에게 "확인해 보세요"라고 넘기지 않는다.

| 재는 것 | 방법 | 기준 |
|---------|------|------|
| 평행투영 | 도로 띠 폭 3곳(높이별) · 모형 좌우변 기울기 · 주택 크기 | 폭이 같고 변이 거의 수직 |
| 화질 | 라플라시안 분산 — **전체 + 구역별** | 직전 배포본 대비 −12% 이내 |
| 개수 | 시설별로 세기 | 시설물 수량표와 일치 |
| 사용자가 짚은 자리 | `cv2.matchTemplate` 로 **배율을 훑어 조각의 위치를 찾는다** | 일치도 0.9 이상이면 확정 |

- **지표가 10% 낮아도 바뀌지 않은 영역을 2배로 확대해 1:1 로 견줄 것.** 입체평면도 −10% 는
  지붕 너와·난간 살·데크 널 이음매가 구분되지 않아 통과시켰고(잔디·수관의 고주파 잡티가
  줄었을 뿐), 조감도 −25% 는 잎이 뭉개진 것이 눈에 보여 기각했다. **수치만으로 판정하지 않는다**
  - **−18% 도 통과시킨 적이 있다**(My 조감도 10회차). 데크 널 개수가 같고 난간 두 가닥이
    여전히 틈을 두고 갈라지며 새순 갈래가 같은 자리에 같은 수만큼 있었다 — 넓은 잔디 면의
    질감 통계가 준 손실이지 물체의 해상이 준 것이 아니었다. **−25% 때는 같은 부위에서 잎이
    뭉개진 것이 한눈에 보였다.** 숫자가 아니라 이 차이를 보고 가른다
- **잴 수 있는 것과 없는 것을 가린다.** 개수·위치·평행투영은 화소가 정확하다. 그러나
  **각도와 밝기 대비는 화소로 재면 오히려 틀린다** — 안내판 각도를 PCA·그래디언트로 재니
  같은 판에서 **6° 와 29°** 가 나왔고, 밝은 판과 밝은 포장의 밝기 차이는 표본 상자를 어디에
  두느냐에 따라 **−36 과 −10** 이 다 나왔다(그늘·질감이 섞인다). 이런 항목은 4~6배로 확대해
  직전 회차와 나란히 놓고 **눈으로 판정하고, 무엇을 보고 그렇게 판단했는지 적는다**
- **기준을 먼저 적고 시작한다.** "①~⑤ 를 다 만족하면 채택, 하나라도 어긋나면 직전 것을 그대로
  둔다"를 정해 두면, 고쳐지긴 했는데 화질을 잃은 회차를 붙잡고 고민하지 않는다
- 회차 파일(`.txt` 프롬프트 + `.png`)을 **모두 남긴다** — 되돌리기도 한 번이고, 무엇이 나아졌는지
  견주려면 앞 회차가 있어야 한다

### 도구

두 갈래가 있고 **OpenAI 쪽이 기본**이다.
...
- **워크트리에서 돌릴 때 두 군데서 걸린다.** 이 스크립트는 `django.setup()` 을 하는데
  ① 시스템 파이썬에 `allauth` 가 없어 죽고 ② 워크트리에는 `.env` 가 없어
  "OPENAI_API_KEY 가 없습니다" 가 나온다. **venv 파이썬 + 본 저장소 `.env` 를 환경변수로**
  넘긴다(값은 출력하지 않는다):

```bash
set -a && . <(grep -E '^OPENAI_API_KEY=' "<본 저장소>/.env" | sed 's/\r$//') && set +a
"<본 저장소>/venv/Scripts/python.exe" gen_note_image_gpt.py …
```

### 인포그래픽만의 검증 — 글자와 사실

위 3절(화소로 재기)에 더해, **글자가 들어가는 그림**은 이것을 본다.
...
`high`($0.041)와 눈에 띄는 차이가 없었다. 덧그린 그림을 마무리할 때만 `xhigh` 를 쓴다(2절).
