...
> **계산형의 규칙 경로는 사실상 죽어 있다.** 서버 채점 기록 47건을 세어 보면
> `rule` 39건이 **전부 빈 답안**이고, 답을 쓴 8건은 계산형을 포함해 모두 `llm` 이다.
> `final_answer_numbers()` 가 모범답안에서 최종 답(0.575)뿐 아니라 중간값
> (20,000 · 500 · 11,500)까지 뽑아내어, 답만 쓴 답안은 통과하지 못한다.
> **고치지 말 것** — 통과시키려 들면 위에서 걷어낸 문자열 비교를 계산형에 다시
> 들이게 된다(0.575와 0.58, 단위 누락, `∴` 없는 답…). 남겨 둔 이유였던 "LLM 산수
> 오류 방지"는 계산 전용 프롬프트가 맡는다 — 배점의 70%를 답 정확성, 30%를 과정에
> 배분한다(답이 맞으면 과정을 생략해도 70%는 준다).
