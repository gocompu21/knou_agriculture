...
- **도형 채우기**(도구 `fill`, 단축키 F, `fillAt`): 누른 면을 설정의 채우기 색·진하기(`SET.fillColor`·`fillOp`)로 칠한다.
  ...
  **채우기 유형**(`SET.fillType` = solid·h·v·d(사선 ／)·x(X자 = 두 사선)·dot(점, 줄마다 반 칸 엇갈림, 반지름 = min(0.28U, 간격×0.13)) · gl(큰 가로세로 격자, 간격 1.5배) · grass(잔디 — 가운데 곧은 잎 + 밑동을 조금 띄운 좌우 긴 잎이 위에서 바깥으로 휜 풀포기를 줄마다 엇갈려, 자리·키를 sin 해시로 늘 같게 흔든다. 좌우 잎이 짧고 한 점에서 나오면 ↓ 화살표로 보였다) · gs(작은 격자, 간격 0.75배 — 절반은 '더 넓게' 요청, 굵기 min(0.11U, 간격×0.16)) — X자는 선 굵기 절반): 수평선·수직선은 면 경로를 clipPath 로 삼아 테두리 상자(`op.bb`) 전체에
