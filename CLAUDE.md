...
- **관리자 사진 저장**: 목록 줄마다 '저장'(스태프만) → 그 카드의 사진을 늘어놓고 하나를
  ...
  - 문제 사진은 여러 장이 세로로 붙은 한 장이라 서버가 **낱장 좌표만** 주고 브라우저가
    canvas 로 자른다 — 서버에 파일을 새로 만들지 않는다. 틈은 "행 전체(92%)가 흰 곳"으로
    본다(사진 속 하늘을 틈으로 잘못 잡지 않도록). 123장 중 121장이 2장, 쇠별꽃 3장,
    털진득찰은 틈이 희지 않아 한 덩어리로 남는다
  - **저장 위치는 브라우저만 정할 수 있다.** Chrome·Edge 는 `showDirectoryPicker` 로
    폴더를 고르고 손잡이를 IndexedDB 에 담아 다음에도 쓴다(문자열이 아니라 localStorage
    에는 못 넣는다). **다운로드·바탕화면·사진 폴더 *자체* 는 브라우저가 거부한다**
    ("이 폴더를 열 수 없음") — 그 안의 하위 폴더는 된다. 거부(AbortError 가 아닌 오류)면
    안내를 띄우고 기본 다운로드 방식으로 돌아간다. 되살린 폴더는 권한이 사라졌을 수
    있어 쓰기 직전에 `requestPermission` 으로 다시 확인한다
