...
세션만). **같은 문항 카드를 쓰고 입력칸 답안지 자리만 사진으로 바꾼다** — 그 문항
답안 박스 둘레를 SVG `viewBox` 로 잘라(사진은 `<image>` 로 깐다) 줄 긋기·✗·물결·
밑줄·✓·번호·점수를 겹치고, 첨삭 글은 **번호를 달아 사진 바로 아래에 전부** 적는다.
총평·항목 판정·모범답안·해설·점수 조정은 카드에 그대로 남는다.

> 처음에는 쪽 전체를 따로 보여 주고 문항 카드를 감췄다. 그러면 사진 위 빈 줄에 들어간
> 첨삭 몇 줄 말고 총평·판정·✓ 첨삭 글이 다 사라져 "입력한 답안 첨삭보다 부실하다"는
> 지적을 받았다. 사진 위 빈 줄에 글을 우겨넣지 말 것 — 모자라 잘린다

- **전체 점수는 첫 쪽 머리(제목) 위 왼쪽**에 손글씨로 적는다(`svHeadHtml`, 사용자 요청).
  제목 줄(y≈230) 위에 둔다 — 밑줄이 제목을 지나가면 취소선처럼 보였다. 60% 넘으면 두 줄 밑줄
...
- 문항 점수는 박스 오른쪽 위 인쇄 배점 '(5.0점)' **왼쪽**에 적는다(겹쳤다)
- 휴대폰에서는 박스가 340px 남짓이라 손글씨가 작다. 사진을 누르면 900px 로 넓혀 옆으로 민다
- 사진에서 박스를 못 찾은 문항은 입력한 답안 첨삭이 그대로 보인다(`.qr.has-crop` 일 때만 감춘다)
- 카드는 `data-qno` 로 찾는다. 한 문항이 두 쪽에 걸치면 사진이 이어 붙는다
