...
- **교수 메모**(`.wq-notes-ov`): 답을 고르면 **문제 사진 위쪽에 흰 반투명 상자**로 겹쳐
  보인다. 답 영역에는 두지 않는다 — 사진을 보며 메모를 읽는 편이 자연스럽다. 77종에
  있고 가장 긴 것이 9줄이라 `max-height: 60%` 로 두어야 잘리지 않는다
...
- 화면: 범위 버튼(전체/출제된 것 우선/오답만) → 사진 + 보기 4개 → 고르면 **사진 위에
  교수 메모(위)와 손그림(오른쪽 아래)**이 뜨고, 아래에 이름(+별)·과·생활형·발생지·
  출제횟수·식별 포인트·유사종 구별·방제·원본 슬라이드
