...
- **교수 메모**(`.wq-notes-ov`): 답을 고르면 **문제 사진 왼쪽 위에 흰 글씨**로 얹힌다
  (배경 상자 없음). 답 영역에는 두지 않는다 — 사진을 보며 메모를 읽는 편이 자연스럽다.
  77종에 있고 가장 긴 것이 9줄이다. 억새·개구리밥처럼 **밝은 사진**이 있어 글자 그림자만
  으로는 안 읽히므로, 글 뒤에만 옅은 어둠을 까는 그라데이션을 함께 쓴다. 이 그라데이션이
  모서리 밖으로 새지 않도록 `.wq-photo-in` 에 `border-radius: 8px; overflow: hidden`
...
- 화면: **탭에 들어오면 바로 '전체' 범위로 첫 문제가 나온다**(`weedAutoStart` — 진입 시와
  ...
  교수 메모(왼쪽 위 흰 글씨)와 손그림(오른쪽 아래)**이 뜨고, 아래에 이름(+별)·과·생활형·
  발생지·출제횟수·식별 포인트·유사종 구별·방제·원본 슬라이드
  - `.wq-actions { display: flex }` 는 `[hidden]` 을 이기므로 `.wq-actions[hidden]
    { display: none }` 을 따로 둬야 한다. 없으면 답을 고르기 전부터 '다음 문제' 버튼이
    보인다(자동 시작을 넣고서야 드러난 문제)
