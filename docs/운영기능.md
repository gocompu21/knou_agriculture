...
자료가 최대 80MB(재배식물생리학)라 그냥 폼으로 보내면 다 올라갈 때까지 화면이
멈춘 듯 보였다. `material_manage.html` 이 XHR 로 보내며 `upload.progress` 로
보낸 양·속도를 막대와 글씨로 보여 준다.

- **전송이 끝나도 바로 끝난 게 아니다** — 서버가 파일을 저장하는 동안 "전송 완료 —
  서버에서 저장하는 중입니다…" 로 둔다(`xhr.upload` 의 load 와 `xhr` 의 load 를
  나눠 쓴다)
- `material_upload` 뷰는 `X-Requested-With: XMLHttpRequest` 면 JSON
  (`{ok, redirect}` 또는 `{ok:false, error}`), 아니면 종전대로 리다이렉트한다.
  `FormData` 가 없는 옛 브라우저는 스크립트가 스스로 물러나 폼 전송이 된다
- 413(용량 초과)은 따로 안내한다. nginx 상한은 `conf.d/upload_size.conf` 의
  `client_max_body_size 100M`. 쓰이는 서버 블록은 `sites-enabled/knou` 이고 거기에
  재정의가 없어 http 블록 값이 그대로 적용된다(`sites-available/default` 의 10M 은
  활성화돼 있지 않은 파일이라 무관). `nginx -T` 로 확인할 것
