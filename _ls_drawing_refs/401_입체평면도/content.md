...
(gpt-image-2.5-sunburst)에 주고 만든 **입체평면도**입니다(18회차).
...
18. 안내판의 **흰 판이 멀리서 흰 점으로 도드라진다**는 지적을 받아, 다시 **13회차로 돌아가**
    ...
    판면 밝기가 **80 : 주변 포장 124** 로 확실히 어두워졌고(17회차는 157 : 127 로 오히려 밝았습니다),
    선명도는 1038 로 17회차와 같습니다
...
| 안내판 판면 | — | ✓ 짙은 나뭇결 목재판 — 주변 포장보다 어둡습니다(80 : 124, 18회차) |
| 글자 | 없음 | ✓ |

남은 차이는 둘입니다. ① **저수지가 도면보다 조금 동쪽·넓습니다**(도면 부지 폭의 14~44%, 그림 약 19~50%). ② **남측 도로가 55% 지점에서 시작**합니다(도면 50%).

비용은 18장 약 $0.84 입니다(16~18회차만 `xhigh` 품질이라 장당 $0.074).

## 프롬프트 — 마지막 회차(18회차, 참고: 색 배치도 + 배식설계도 + 13회차 그림)

13회차 그림을 함께 주고 고칠 셋만 적은 것입니다. 1~17회차 프롬프트는 뺐습니다.
    ...
    Three things are wrong. Fix exactly these three. Change nothing else at all.
    ...
    DO NOT TOUCH THE OTHER SIGNBOARDS. The small blank white boards on the five timber viewing platforms of the boardwalk, and the one at the bird-watching hut, MUST ALL STAY - six boards in all, each now with a dark timber panel, at the same places, the same sizes and the same square alignment. Do not delete them, do not move them and do not turn them into benches. The large blank boards on the two entrance plazas also stay. Only the three frames on the gathering plaza change.

    FIX 3 — THE BOARD FACES ARE DARK TIMBER, NOT WHITE.
    Every signboard in the picture - the 5 small boards on the 5 timber viewing platforms, the 1 board at the bird-watching hut, and the large boards on both entrance plazas - currently has a BRIGHT WHITE face inside its timber frame, so it stands out as a white dot. Replace that white face with a panel of DARK, GRAIN-TEXTURED TIMBER. The panel must be DARKER than the grass and darker than the block paving around it; never white, never light grey, never pale. Put only a few very faint lines on it and NO letters and NO text of any kind. Keep the timber frame and its legs exactly as they are - you are changing only the COLOUR AND MATERIAL OF THE PANEL, you are NOT removing any signboard. All of them still stand where they stand.
