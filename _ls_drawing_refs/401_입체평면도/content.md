...
(gpt-image-2.5-sunburst)에 주고 만든 **입체평면도**입니다(13회차).
...
같은 그림이었는데 11~13회차에 질감만 바꿨습니다(아래 참조).
...
13. 11회차 그림으로 돌아가 **"그 줄을 세어 하나, 둘, 셋이 되도록 세 번째 안내판을 그려라"** 라고
    적으니 3개가 됐고 마운딩도 잔디 언덕이 됐습니다(13회차). 이것을 채택했습니다
...
셋이 되도록 세 번째를 그려라"** 라야 됐습니다(13회차).

**실사를 주문하면 모델이 원근으로 되돌아가기 쉽습니다.** 11회차에 질감만 바꾸면서도 평행투영
문장을 통째로 남겨 두었고, 회차마다 화소로 재서 확인했습니다 — 동측 도로 띠 왼쪽 끝이
10회차 1351·1349·1361px, 13회차 1352·1356·1361px 로 같고, 모형 왼쪽 변 기울기도
10회차 −18px, 13회차 −16px(730px 기준)로 같습니다.

## 도면과 대조 (13회차)

| 항목 | 도면 | 그림 |
| --- | --- | --- |
| 투영 | 평행투영(축측투영) | ✓ 동측 도로 띠 왼쪽 끝이 1352·1356·1361px(높이별) · 모형 왼쪽 변이 730px 내려가는 동안 16px · 주택 8채가 같은 크기 |
...
| 배식 | 동선 서쪽 메타세쿼이아 열식 · 동쪽 마운딩에 소나무 7 + 철쭉 · 도로변 은행나무 열식 · 산림지구 다층림 · 저수지·관찰로 서쪽은 열린 잔디 | ✓ 마운딩은 잔디 언덕에 소나무 7주(13회차) |
| 글자 | 없음 | ✓ |
...
## 프롬프트 — 마지막 회차(13회차, 참고: 색 배치도 + 배식설계도 + 11회차 그림)

앞 회차 그림을 함께 주고 고칠 두 가지만 적은 것입니다. 1~12회차 프롬프트는 뺐습니다.

    The FIRST attached image is the colour-coded site plan of a small Korean ecological park. The SECOND is its hand-drawn planting plan. The THIRD attached image is a PHOTOREALISTIC axonometric render of the same park. Its LAYOUT, its CAMERA, its PARALLEL PROJECTION, its MATERIALS, its TEXTURES and its LIGHTING are all CORRECT and must be reproduced exactly. It stays photorealistic.

    Two small things are wrong. Fix only these. Move nothing else, add nothing else, remove nothing else, and do not change the render style.

    FIX 1 — THERE MUST BE THREE SIGNBOARDS ON THE GATHERING PLAZA, NOT TWO.
    Find the gathering plaza: the block-paved square immediately to the right of the timber boardwalk ring, the one with a single tree standing in a square metal grate. Along its LEFT edge, where the paving meets the grass, two blank timber signboards stand one behind the other, facing the wetland.
    DRAW A THIRD ONE. Place it in the same line, below the lower of the two, the same size and the same design as the other two, standing on the paving and facing the wetland the same way, with the same spacing between it and the board above it as there is between the existing pair. When you are finished, counting down that edge must give one, two, THREE separate signboards, each a timber frame with a plain blank white panel. The tree in its grate stays exactly where it is, to the right of the boards.

    FIX 2 — THE MOUND IS A RAISED GRASSY HILL, NOT A FLAT BED.
    East of the straight block-paved path, in the open lawn, the long planted feature must read as a MOUND: ground that swells upward into a low elongated grassy hill, its near slope catching the sun and its far slope in shadow, its turf continuous with the lawn around it and its edges melting into that lawn. In the third image it has become a flat planting bed with a clipped hedge border and bare soil — remove the hedge border and the bare soil, cover it in the same real turf as the lawn, and lift it into a rounded hill. The EXACTLY 7 pines and the broad masses of azaleas stay where they are, now growing out of the grassy slope.
    ...
    - One natural sun from the upper left, soft realistic shadows, natural colour balance. No sky, no horizon, plain neutral background.
    ...
    EVERYTHING ELSE IS EXACTLY AS IN THE THIRD IMAGE: the stream across the top with wooded hillside beyond; the reservoir with its willows, reed beds and shoreline stones; the wetland pond; ONE closed angular ring of raised timber boardwalk with EXACTLY 5 viewing platforms, each with one blank board; EXACTLY 1 bird-watching hut on the reservoir's lower-right shore; the rest area with EXACTLY 1 pergola, 2 benches and 1 litter bin under shade trees; EXACTLY 1 tree in a grate on the gathering plaza; EXACTLY 2 trees in grates and 1 blank board on the top-right entrance plaza; EXACTLY 2 trees in grates and 1 blank board on the bottom entrance plaza — FIVE tree grates in all; ONE single row of about 12 dawn redwoods on the grass immediately LEFT of the straight path; ONE single row of ginkgos inside the right-hand road continuing inside the bottom road; open mown lawn west of the ring and west of the reservoir with only three or four isolated specimen trees; the dense multi-layered woodland only in the lower left and lower middle.
