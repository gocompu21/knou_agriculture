'성운' 탭의 **답안지 II · 기본설계도**와 **답안지 III · 배식설계도**, 문제지 조건을 GPT 이미지 모델(gpt-image-2.5-sunburst)에 주고 만든 **입체 조감도**입니다(2026년 9월 18일, 5회차).
...
| 진입광장 2 — 북동 모서리 · 남쪽(남동 근처), 종합안내판 1 · 수목보호대 2씩, 소형고압블록 | ✓ (북동 광장은 4·5회차에 고침 — 도로로 바로 열림, 안내판 서쪽 변, 보호대 2) |
| 두 진입광장을 잇는 남북 동선, 서쪽 메타세쿼이아 열식, 동쪽 마운딩에 소나무 7 · 철쭉, 도로변 은행나무 열식 | ✓ (4회차에 고침) |
| 글자 없음 | ✓ |

비용은 3장 약 $0.12 입니다.

## 프롬프트 — 마지막 회차(5회차, 참고: 기본설계도 + 4회차 그림)

앞 회차 그림을 함께 주고 "그것만 고치고 나머지는 그대로"라고 한 것입니다. 1~4회차 프롬프트는 뺐습니다.

    The FIRST attached image is the site plan of a small ecological park (north is up). The SECOND attached image is a 3D rendering of it that is correct in layout and camera. Reproduce the second image EXACTLY — same camera, pond, wetland, boardwalk loop, forest, plazas, paths, mound with pines, stream, roads, houses, lighting — and change ONLY the NORTH-EAST ENTRANCE PLAZA:

    Inside the block-paved plaza there must be EXACTLY 2 square tree grates, each with one zelkova tree: one near the plaza's NORTH part and one near its SOUTH part, both roughly in the middle of the plaza's width. Remove every other tree grate from the plaza. The ginkgo trees along the north edge of the site stand in the planted strip NORTH of the plaza, in lawn/soil, NOT in tree grates and NOT on the paving. The blank signboard stays on the plaza's west edge; the plaza still opens directly onto the east road.
