...
## 동측 완충녹지를 제자리로 (2026-09)

주차장 오른쪽 경계가 이상하다는 지적을 받고 **도면을 다시 쟀습니다.** 답안 도면의
회양목 띠는 **x 52.9~55.0, y 12.7~28.1** 로 동측 경계에 **붙어** 있는데, 모형은
52.0~53.8 에 띄워 놓아 경계 쪽에 아무것도 없는 아스팔트 띠가 남아 있었습니다.

띠를 경계에 붙이자 그 위(y 2.8~12.7)와 아래(y 28.1~37.2)의 틈이 제 구실을 합니다 —
도면에 ▶ 로 표시된 **차량입구(북)·차량출구(남)** 이고, 그리로 아스팔트가 빠져나가
도로에 붙습니다. 실사 그림에서도 그 두 곳이 진입로로 읽히게 했습니다.

> 이 잘못은 **위에서 내려다보는 평면도·축측도에서는 눈에 안 띄었습니다.** 경계 쪽
> 아스팔트가 그냥 주차장의 일부로 보였기 때문입니다. 비스듬히 보는 조감도에서야
> "저기에 왜 아무것도 없지" 가 드러났습니다 — 40m 짜리 기둥 때와 같습니다.
