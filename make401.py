...
SPECIES = {
    ...
    '찔레':     (0.50, 1.20, (0.15, 0.29, 0.11), (0.93, 0.91, 0.84), 0.34),   # H0.6 x W0.5
...
TREE_SPEC = {
    ...
    '생강나무':     ('ball', 0.72, (0.26, 0.38, 0.13)),   # H2.0 x R4 — 소교목
}
...
def build():
    ...
    # **북 · 동 · 남 세 면을 은행나무로 두른다**(완충식재 — 주택가·도로와의 기능 분리).
    #   수량표는 18 주이나 도면의 열식을 그대로 따르면 그보다 많다 — 수량보다
    #   '둘러 심는다'는 뜻이 중요하다(사용자 지시)
    gink = 0
    for y in [1.6 + i * 2.1 for i in range(29)]:          # 동측 도로변
        if y < D - 0.8 and free(88.6, y, m=0.4):
            plant(88.6, y, '은행나무'); gink += 1
    for x in [44.0 + i * 3.2 for i in range(15)]:         # 북측 경계
    ...
    # 8-c) **생강나무 4 · 찔레 140 — 배식설계도 인출선 자리.**
    #   생강나무는 기존수림 동쪽 가장자리(관찰로 서쪽 잔디)에 넷.
    #   찔레는 **관찰로 루프 안쪽 서편**에 북동–남서로 누운 덤불 띠다
    #   (물가가 아니라 안쪽 마른 가장자리 — 도면이 그리 가리킨다)
    for x, y in ((16.7, 24.6), (16.8, 27.8), (15.6, 29.9), (17.9, 31.7)):
        if free(x, y, m=0.8):
            plant(x, y, '생강나무')
    br = []
    for _ in range(9000):                             # 수량표대로 **140 주**에서 멈춘다
        if len(br) >= 140:
            break
        t = random.random()
        cx = 29.5 + (24.0 - 29.5) * t                 # (29.5,24) → (24,35)
        cy = 24.0 + (35.0 - 24.0) * t
        x = cx + random.uniform(-1.0, 1.0)
        y = cy + random.uniform(-0.8, 0.8)
        if _inside((x, y), RO_IN) and free(x, y, m=0.3) and not _inside((x, y), MARSH):
            br.append((x, y))
    scatter_species(br, ['찔레'], height)
    print('생강나무 4 · 찔레', len(br), '주')

    # 8-b) **북측 경계 식재 — 길 쪽부터 켜를 이룬다.**
    #   도면을 보면 **길을 따라 진달래 띠 → 그 뒤 아교목 → 맨 뒤(북쪽) 은행나무** 다.
    #   처음에 거꾸로 두었다(진달래를 뒤에, 교목을 경계선에) — 길에서 보면
    #   낮은 것부터 높은 것으로 켜가 서야 자연스럽다
    NB = (44.0, 89.0)
    # ① 길을 따라 진달래 띠 (동측 통로·모임광장의 북쪽 변 y=13 에 붙여)
    nb = [(random.uniform(*NB), random.uniform(10.6, 12.9)) for _ in range(1100)]
    scatter_species([q for q in nb if free(*q, m=0.3)], ['진달래', '진달래', '철쭉'], height)
    # ② 가운데 아교목 — 산벚나무 9 · 산딸나무 5
    for i in range(9):
        x, y = 46.0 + i * 4.6, 8.4 + (0.7 if i % 2 else -0.5)
        if free(x, y, m=1.0):
            plant(x, y, '산벚나무')
    for q in spots(5, (50.0, 86.0, 6.6, 9.6), lambda x, y: free(x, y, m=1.4), gap=6.0):
        plant(*q, '산딸나무')
    # ③ 맨 뒤(북쪽) 은행나무는 위의 '세 면 두르기'에서 한 줄로 넣는다
    # ④ 못·실개천 물가 — 메타세쿼이아 5 · 갯버들 · 부들 · 갈대
    for q in spots(5, (38.0, 50.0, 0.9, 5.5), lambda x, y: free(x, y, m=1.2), gap=2.8):
        plant(*q, '메타세쿼이아')
    wb = [(random.uniform(24.0, 46.0), random.uniform(0.6, 3.2)) for _ in range(500)]
    scatter_species([q for q in wb if free(*q, m=0.3)], ['갯버들', '부들', '갈대'], height)
    ...
    #   몰아 두어 위쪽이 비고 아래가 답답했다. 마운딩(27~48)에 맞춰 올린다
    for q in spots(7, (76.0, 85.0, 26.0, 46.0),
